...
此前保留的图片重建占位已全部按本地 Word 原稿和本地图片清单完成处理：函数图像、结构示意图、流程图等需要保留视觉信息的内容作为公开图片资源引用；纯文字、公式和表格已人工转写为 Markdown/LaTeX，并保持与正文一致的排版。后续若发现转写误差，会继续依据 Word 可见裁剪图修正。
