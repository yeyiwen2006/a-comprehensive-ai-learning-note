...
- 2026年9月2日发布 v2.0 版本（当前版本）。
  ...
  更新内容：对第2部分（强化学习）、第3部分（大语言模型）进行了微调，删除了一些非主流或学习价值不高的内容；对原第4部分、第5部分（涉及大模型智能体、扩散模型、多模态生成、具身智能、世界模型等）的内容进行了增删和修改，将其重构为当前的第4部分（大模型智能体）、第5部分（扩散模型与多模态生成）、第6部分（具身智能与世界模型），新增了业界的一些新方向、新趋势，删除了一些非主流或学习价值不高的内容，修改了部分文字表述。此外还修改了 `README.md`、初学者学习路径建议等。
