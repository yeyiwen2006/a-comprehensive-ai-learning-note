...
\item
  thedotmack. (n.d.). \href{https://github.com/thedotmack/claude-mem}{claude-mem GitHub repository}. 访问于2026-09-02.
\item
...
短期记忆是大模型执行任务时可利用的当前任务上下文，对应输入给模型的消息、工具结果和其他上下文Token。推理系统通常使用KV Cache缓存已经处理过的Token在各注意力层中的Key/Value状态，以避免自回归生成时重复计算；KV Cache是推理优化和运行时状态，不等同于上下文本身，关闭它不会消除上下文信息，只会增加重复计算。
...
summary并非简单把长文本变成短文本，否则会丢失大量细节。它应尽量保留继续任务所需的目标、约束、已完成工作和未解决问题。OpenAI Codex在上下文Token超过阈值后，会调用Responses API的\texttt{/responses/compact}端点，用一组更小但能代表原对话的input items替换旧输入；其中包含带加密内容的\texttt{type=compaction}项，以保留模型对原对话的潜在理解。早期Codex曾使用摘要消息替换旧输入，但当前实现并不是固定的``小项目列表''。
...
\item
  温睦宁、林江浩、张伟楠、俞勇. (2025). \href{https://haa.boyuai.com/}{《动手学大模型智能体》}. 人民邮电出版社. ISBN 978-7-115-68638-1.
\item
  OpenClaw. (n.d.). \href{https://github.com/openclaw/openclaw}{OpenClaw GitHub repository}. 访问于2026-09-02.
\item
  thedotmack. (n.d.). \href{https://github.com/thedotmack/claude-mem}{claude-mem GitHub repository}. 访问于2026-09-02.
\item
  Hugging Face. (n.d.). \href{https://huggingface.co/docs/transformers/main/kv_cache}{Cache strategies}. Transformers Documentation. 访问于2026-09-02.
\item
