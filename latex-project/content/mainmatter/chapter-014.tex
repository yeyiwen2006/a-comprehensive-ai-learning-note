...
\tightlist
\item
  \textbf{混合采样}：从两个池子中按照一定比例采样构建训练 Batch。
  ...
  \end{itemize}
\item
  \textbf{SAC 更新}：利用这个混合 Batch 更新 Actor 和 Critic 的参数（梯度下降）。
...
\tightlist
\item
  \textbf{收集碎片}：从离线轨迹中随机切出三点 \((s_i,s_k,s_j)\)。
\item
  \textbf{局部对齐}：检查当前的神经网络是否满足 \(Q(i,j)\approx Q(i,k)\cdot Q(k,j)\)。
\item
  \textbf{择优录取}：通过 \(\kappa\) 权重，如果发现某一种 \(k\) 的切分方式能产生更大的价值（代表更短的路径），就让 \(Q(i,j)\) 向这个更大的值靠拢。
\item
  \textbf{由小及大}：优先训练距离近的点对，逐渐建立起对整个大地图（长时序）的价值感知。
\end{enumerate}
