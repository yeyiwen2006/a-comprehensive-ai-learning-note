...
值得注意的是，训练过程对数值精度较敏感，不能简单地把全部权重、梯度和优化器状态都改成低位整数。最常见的部署流程仍是训练后量化（PTQ），但``训练时不应量化''并不成立：量化感知训练（QAT）会在前向过程中模拟量化误差，通常保留更高精度的主权重或梯度；QLoRA 等量化微调方法则冻结低比特基座权重，并以更高精度训练适配器和优化器状态。是否采用这些方法应结合硬件、精度目标和训练稳定性评估。对于物理模拟等对数值误差高度敏感的任务，也应先验证量化误差是否可接受。
