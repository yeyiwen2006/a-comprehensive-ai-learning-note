...
2017年，Google发表论文《Attention Is All You Need》，提出了 Transformer。它是首个完全以注意力构建、且不依赖循环或卷积的主流序列建模架构。注意力机制本身并非始于 Transformer：Bahdanau 等人在此前的神经机器翻译工作中已经把可学习的软对齐注意力用于循环编码器---解码器；Transformer 的关键推进，是把注意力从辅助组件提升为整个架构的核心。它采取了``选择性聚焦''的方法，即保留上下文中的信息，并由模型自主聚焦于当前最相关的部分。例如：人的面前有红色的咖啡杯，也有一些报纸、论文、书。在非自主性的情况下，我们的视觉系统往往被红色的咖啡杯吸引；而我们也可以自主性地、有意识地决定去读书，把注意力放在书上。
