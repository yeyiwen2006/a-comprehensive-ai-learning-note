...
\subsubsection{（一）问题背景}\label{ux4e00ux95eeux9898ux80ccux666f-17}}
...
\item
  Gemini Robotics Team, Choromanski, K., Devin, C., Du, Y., Dwibedi, D., et al.~(2025). \href{https://arxiv.org/abs/2512.10675}{Evaluating Gemini Robotics Policies in a Veo World Simulator}. arXiv:2512.10675.
\item
  Wang, S., Liu, S., Ye, W., You, J., \& Gao, Y. (2024). \href{https://arxiv.org/abs/2403.00564}{EfficientZero V2: Mastering Discrete and Continuous Control with Limited Data}. arXiv:2403.00564.
\item
  Zhou, G., Swaminathan, S., Raju, R. V., Guntupalli, J. S., Lehrach, W., Ortiz, J., Dedieu, A., Lázaro-Gredilla, M., \& Murphy, K. (2024). \href{https://arxiv.org/abs/2410.05364}{Diffusion Model Predictive Control}. arXiv:2410.05364.
\item
  Yang, Y., Liu, J., Zhang, Z., Zhou, S., Tan, R., Yang, J., Du, Y., \& Gan, C. (2025). \href{https://arxiv.org/abs/2507.12508}{MindJourney: Test-Time Scaling with World Models for Spatial Reasoning}. arXiv:2507.12508.
\item
  Yu, S., Zhang, Y., Wang, Z., et al.~(2026). \href{https://arxiv.org/abs/2602.08236}{When and How Much to Imagine: Adaptive Test-Time Scaling with World Models for Visual Spatial Reasoning}. arXiv:2602.08236.
\item
  Zhang, M., Zhuo, L., Tan, T., et al.~(2025). \href{https://arxiv.org/abs/2511.15407}{IPR-1: Interactive Physical Reasoner}. arXiv:2511.15407.
\end{itemize}
...
\hypertarget{ux4e09pelican-unified-1.0ux5c06ux9884ux6d4bux4e0eux52a8ux4f5cux5728-diffusion-ux4e2dux5171ux540cux89e3ux7801}{%
\subsection{三、Pelican-Unified 1.0：将预测与动作在 Diffusion 中共同解码}\label{ux4e09pelican-unified-1.0ux5c06ux9884ux6d4bux4e0eux52a8ux4f5cux5728-diffusion-ux4e2dux5171ux540cux89e3ux7801}}

\hypertarget{ux4e00ux6a21ux578bux67b6ux6784ux4e0eux5de5ux4f5cux6d41-1}{%
\subsubsection{（一）模型架构与工作流}\label{ux4e00ux6a21ux578bux67b6ux6784ux4e0eux5de5ux4f5cux6d41-1}}

对于理解模块，用一个视觉-语言模型（初始化自 Qwen3-VL 4B），接收多模态上下文，包括观察、历史动作和语言指令，生成 Chain-of-Thought。论文将这里的 CoT 当成中间决策结构。它应该包含任务意图、物理约束、未来后果和动作选择。输出的最后一个 token 不解码，直接用其最后一层的隐状态，经过一个投影层，成为后续视频生成和动作生成共同依赖的稠密状态。

对于统一生成模块（Unified Future Generator），用一个视频生成模型（初始化自 Wan2.2-5B）微调，其任务是同时生成两种未来：未来视频，即机器人接下来会看到什么；未来动作，即机器人接下来该怎么动。两者共用同一个 Diffusion Transformer 共同去噪，因此未来想象可以影响动作，动作动态也可以约束未来视频。最后再用两个输出头分别预测视频和动作的速度场。

\hypertarget{ux4e8cux8badux7ec3ux65b9ux6cd5-2}{%
\subsubsection{（二）训练方法}\label{ux4e8cux8badux7ec3ux65b9ux6cd5-2}}

训练时，将未来视频编码到 VAE latent 空间：

\[
x^v = E_{\mathrm{vae}}\left(v_{t:t+H}\right)
\]

其中 \(v_{t:t+H}\) 是真实未来视频，\(E_{\mathrm{vae}}\) 是视频自编码器，\(x^v\) 是视频 latent。

动作也被归一化成连续表示：

\[
x^a = \mathrm{Norm}\left(a_{t:t+H}\right)
\]

其中 \(a_{t:t+H}\) 是未来动作片段，\(x^a\) 是归一化后的动作表示。

然后视频 latent 和动作轨迹都会被加噪，分别先通过模态专属 embedding 后加噪送入 DiT，用流匹配损失进行训练。文本、视频、动作三种损失共同训练。

\hypertarget{ux56dbux7269ux7406-token-ux7684ux7edfux4e00ux81eaux56deux5f52ux9884ux6d4b}{%
\subsection{四、物理 token 的统一自回归预测}\label{ux56dbux7269ux7406-token-ux7684ux7edfux4e00ux81eaux56deux5f52ux9884ux6d4b}}

具身智能中，未来视觉状态和动作本质都是高维连续信号。PAR、PhysGen 等论文及 NVIDIA 的 DreamDojo 等提出了状态与策略统一编码于``物理 token''中，统一进行自回归生成，再在下游解码为状态与策略输出的范式。

\hypertarget{ux4e00ux6838ux5fc3ux601dux60f3-28}{%
\subsubsection{（一）核心思想}\label{ux4e00ux6838ux5fc3ux601dux60f3-28}}

将过去的视频和动作均编码为统一的物理 token。设 \(t\) 时刻的视频帧 token 集合为 \(o_t\)（PhysGen 中每个 \(o_t\) 包含 360 个 token），动作 token 集合为 \(a_t\)（PhysGen 中每个 \(a_t\) 包含 8 个 token），则上下文序列为：

\[
\left[\mathrm{prompt}, o_1, a_1, o_2, a_2, \ldots, o_t, a_t\right]
\]

模型联合预测下一步的 \([o_{t+1}, a_{t+1}]\)。预测出的 \([o_{t+1}, a_{t+1}]\) 再通过扩散解码，分别得出连续的未来状态值（视频）和要采取的动作值。

当然具体来说，\(o_t\) 和 \(a_t\) 也不一样：\(o_t\) 内部的 token 可以并行生成；\(a_t\) 内部则按时间顺序严格串行，但利用 multi-token prediction 的方法，以在动作块的内部拉长微观的规划视野，即便预判 2 到 3 步极其微小的机械臂位姿，也能让自回归生成的动作轨迹更加连贯，有效避免了控制上的``短视''和抖动。
...
该阶段只训练语言 tokenizer、视觉 tokenizer（3D-VAE）、自回归 Transformer 主干网络，以及帧生成器（Frame Diffusion），动作模块不参与。
...
此时，模型会增加一个用于编码动作的 Action Tokenizer（一个 MLP），以及一个轻量级的动作解码器（Action-DiT）。微调时，模型被训练为一旦看到 BOA token，后面就按``360 个视频帧 token + 8 个动作 token''交替生成，系统也会按 token 数量交替分发给不同解码器。

\hypertarget{ux4e94last-r1ux5bf9ux4e16ux754cux9690ux72b6ux6001ux63a8ux7406-ux52a8ux4f5cux751fux6210ux601dux7ef4ux94feux8fd0ux7528rl}{%
\subsection{五、LaST-R1：对``世界隐状态推理-动作生成''思维链运用RL}\label{ux4e94last-r1ux5bf9ux4e16ux754cux9690ux72b6ux6001ux63a8ux7406-ux52a8ux4f5cux751fux6210ux601dux7ef4ux94feux8fd0ux7528rl}}

\hypertarget{ux4e00ux95eeux9898ux80ccux666fux4e0eux6838ux5fc3ux601dux60f3-3}{%
\subsubsection{（一）问题背景与核心思想}\label{ux4e00ux95eeux9898ux80ccux666fux4e0eux6838ux5fc3ux601dux60f3-3}}
...
\hypertarget{ux4e8cux6838ux5fc3ux7b97ux6cd5lapo}{%
\subsubsection{（二）核心算法：LAPO}\label{ux4e8cux6838ux5fc3ux7b97ux6cd5lapo}}
...
\(\mathcal{L}_{\mathrm{end}}\)则自适应地决定合适结束物理隐状态推理，执行动作，使得物理推理不只是一步预测，而成为一个可以被动态调节的推理过程。具体而言，\(\mathcal{L}_{\mathrm{end}}\)针对\texttt{\textless{}latent\_end\textgreater{}}这个离散词表内的token，用PPO风格的损失，核心逻辑是\(\hat{A}_t>0\)时奖励当前时刻的停止决策，\(\hat{A}_t<0\)时抑制当前时刻的停止决策。这里\texttt{\textless{}latent\_end\textgreater{}}的生成方式是在每若干步隐状态推理后进行一次在离散词表内的预测，一旦预测到\texttt{\textless{}latent\_end\textgreater{}}或达到隐状态推理最大步数，则停止隐状态推理，生成动作。
...
\item
  Zhang, Y., Chen, Y., Liu, C., Ding, Z., Xu, J., Zou, S., et al.~(2026). \href{https://arxiv.org/abs/2605.15153}{Pelican-Unified 1.0: A Unified Embodied Intelligence Model for Understanding, Reasoning, Imagination and Action}. arXiv:2605.15153.
\item
  Song, Z., Qin, S., Chen, T., Lin, L., \& Wang, G. (2025). \href{https://arxiv.org/abs/2508.09822}{Physical Autoregressive Model for Robotic Manipulation without Action Pretraining}. arXiv:2508.09822.
\item
  Song, Z., Li, Q., Qin, S., et al.~(2026). \href{https://arxiv.org/abs/2603.00110}{Learning Physics from Pretrained Video Models: A Multimodal Continuous and Sequential World Interaction Models for Robotic Manipulation}. arXiv:2603.00110.
\item
...
\item
  Chen, H., Liu, J., Yan, Z., et al.~(2026). \href{https://arxiv.org/abs/2604.28192}{LaST-R1: Reinforcing Robotic Manipulation via Adaptive Physical Latent Reasoning}. arXiv:2604.28192.
\end{itemize}
...
\subsubsection{（四）损失函数}\label{ux56dbux635fux5931ux51fdux6570-2}}
