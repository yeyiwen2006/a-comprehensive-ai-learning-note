...
但如果考虑到与物理世界交互，上述条件将不再成立。模型需要能对与模型互动的环境进行建模，以预测行动带来的后果。在数字世界中，环境可以通过文本token的形式被编码，LLM已经承担了这一功能；但在物理世界中，例如我们希望机器人在现实世界中顺利执行新的任务，就要求模型能从直接来自现实世界的连续高维数据（像素、传感器数据）中对环境进行建模，理解事物之间相互影响的物理规律和行动带来的结果（即物理世界的因果关系）。这既是人类想象力与创造力的基石，又带来了在现实世界中进行规划的能力。
...
\item
  赋予模型对物理世界的直觉推理能力和对三维空间的认知能力。纵观人类，美术作品和文创产品的创造、建筑物的设计、显微镜和各类仪器等科学工具的发明、DNA双螺旋结构的发现等，都离不开对三维空间和物理直觉的推理。部分观点认为这是实现人类水平智能和赋予智能``接地性''必不可少的部分。
\item
