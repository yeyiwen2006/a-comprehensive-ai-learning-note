...
\hypertarget{ux4e00ux4e3aux4ec0ux4e48ux9700ux8981ux4e16ux754cux6a21ux578b}{%
\subsection{一、为什么需要世界模型？}\label{ux4e00ux4e3aux4ec0ux4e48ux9700ux8981ux4e16ux754cux6a21ux578b}}
...
\item
  赋予模型对物理世界的直觉推理能力和对三维空间的认知能力。纵观人类，美术作品和文创产品的创造、建筑物的设计、显微镜等工具的发明、DNA双螺旋结构的发现等，都离不开对三维空间和物理直觉的推理。部分观点认为这是实现人类水平智能和赋予智能``接地性''必不可少的部分。
\item
  生成世界将成为一种新的范式，赋能创意、游戏等产业和内容生成行业。
\item
  成为具身智能重要的组成部分和训练环境。一方面，赋予具身智能在物理世界中行动前预测后果的能力，从而使智能体可以进行规划；另一方面，通过生成模拟数据解决具身智能训练数据不足的难题，对具身智能的发展起着关键的作用。
\end{enumerate}
...
\begin{itemize}
\item
  \textbf{方块三视图投影（Cube 3-view Projection）}：提供一个相连方块堆的等距视角（斜视图）和两个正交视图（如正视图、右视图），要求模型预测并描述从另一个完全未见过的视角（如后视图）看到的方块数量。解决该任务需要模型先在大脑中重建出方块的完整 3D 结构，然后再进行心理旋转或投影，这与人类的视觉心智表征过程高度一致。
\item
  \textbf{真实世界空间推理（Real-world Spatial Reasoning）}：给定一个真实房间或场景的多个不同视角图像，询问摄像机、物体或区域之间的相对位置关系（例如``拍第二张照片时黑门在我的哪个方向''）。成功回答需要模型从这几个有限的 2D 视角中，拼凑并构建出一个连贯的 3D 空间心智模型。
\end{itemize}
...
\begin{itemize}
\item
  \textbf{折纸（Paper Folding）}：给定一系列纸张折叠和打孔的步骤，要求预测纸张完全展开后的孔洞分布情况。这要求模型能够在内部模拟纸张逐步展开的物理过程，高度依赖基于视觉经验的几何对称性和空间变换先验知识。
\item
  \textbf{多步操作（Multi-hop Manipulation）}：给出一个包含不同形状、颜色物体的初始场景，并提供一系列指令（如添加、移除、交换颜色等）。最终询问经过这些操作后的空间布局属性（例如某物体的左边是什么）。这极大地考验了模型对物体状态的持续跟踪以及动态空间理解能力。
\item
  \textbf{小球追踪（Ball Tracking）}：要求模型推断一个匀速运动的小球在碰到实心墙壁发生理想镜面反射后的轨迹，并预测它最先进入顶部哪个编号的洞中。这直接评估了模型对基础物理动态过程的模拟预测能力。
\item
  \textbf{迷宫（Maze）与推箱子（Sokoban）}：这两项是经典的网格世界任务，要求模型理解当前状态并模拟出从起点到终点（或将箱子推到目标点）的连续动作序列。
\end{itemize}
...
\tightlist
\item
  Brooks, T., Peebles, B., Holmes, C., et al.~(2024). \href{https://openai.com/index/video-generation-models-as-world-simulators/}{Video generation models as world simulators}. OpenAI.
\item
  OpenAI. (2025). \href{https://openai.com/index/sora-2-system-card/}{Sora 2 System Card}. OpenAI.
\item
  Bruce, J., Dennis, M., Edwards, A., et al.~(2024). \href{https://arxiv.org/abs/2402.15391}{Genie: Generative Interactive Environments}. ICML.
\item
  Parker-Holder, J., Ball, P., Bruce, J., et al.~(2024). \href{https://deepmind.google/blog/genie-2-a-large-scale-foundation-world-model/}{Genie 2: A large-scale foundation world model}. Google DeepMind.
\item
  Parker-Holder, J., \& Fruchter, S. (2025). \href{https://deepmind.google/blog/genie-3-a-new-frontier-for-world-models/}{Genie 3: A new frontier for world models}. Google DeepMind.
\item
...
\item
  Team Seedance, Chen, D., Chen, L., et al.~(2026). \href{https://arxiv.org/abs/2604.14148}{Seedance 2.0: Advancing Video Generation for World Complexity}. arXiv:2604.14148.
\item
  Google DeepMind. (2025). \href{https://deepmind.google/models/veo/}{Veo 3.1}. Google DeepMind model page.
\item
  Alonso, E., Jelley, A., Micheli, V., et al.~(2024). \href{https://arxiv.org/abs/2405.12399}{Diffusion for World Modeling: Visual Details Matter in Atari}. NeurIPS.
\item
  Robbyant Team: Gao, Z., Wang, Q., Zeng, Y., et al.~(2026). \href{https://arxiv.org/abs/2601.20540}{Advancing Open-source World Models}. arXiv:2601.20540.
\item
  Li, L., Zhang, Q., Luo, Y., et al.~(2026). \href{https://arxiv.org/abs/2601.21998}{Causal World Modeling for Robot Control}. arXiv:2601.21998.
\item
  NVIDIA: Agarwal, N., Ali, A., Bala, M., et al.~(2025). \href{https://arxiv.org/abs/2501.03575}{Cosmos World Foundation Model Platform for Physical AI}. arXiv:2501.03575.
\item
...
\item
  Google DeepMind. (2026). \href{https://deepmind.google/models/gemini-omni/}{Gemini Omni}. Google DeepMind model page.
\item
  World Labs team. (2025). \href{https://www.worldlabs.ai/blog/rtfm}{RTFM: A Real-Time Frame Model}. World Labs Research Preview.
\item
  Wu, F., Wei, J., Li, R., Xu, Y., Li, J., Ye, D., \& Lin, G. (2025). \href{https://arxiv.org/abs/2512.02793}{IC-World: In-Context Generation for Shared World Modeling}. arXiv:2512.02793.
\item
...
\item
  World Labs team. (2025). \href{https://www.worldlabs.ai/blog/marble-world-model}{Marble: A Multimodal World Model}. World Labs.
\item
...
\item
  Ren, Z., Wei, Y., Guo, X., Zhao, Y., Kang, B., Feng, J., \& Jin, X. (2025). \href{https://arxiv.org/abs/2501.09781}{VideoWorld: Exploring Knowledge Learning from Unlabeled Videos}. arXiv:2501.09781.
\item
...
\hypertarget{ux7ed3ux5408ux4e16ux754cux6a21ux578bux7684ux81eaux4e3bux5b66ux4e60ux667aux80fdux7cfbux7edfux5c55ux671bux8bbaux6587}{%
\section{结合世界模型的自主学习智能系统展望（论文）}\label{ux7ed3ux5408ux4e16ux754cux6a21ux578bux7684ux81eaux4e3bux5b66ux4e60ux667aux80fdux7cfbux7edfux5c55ux671bux8bbaux6587}}
...
\hypertarget{ux4e00a-b-mux81eaux4e3bux5b66ux4e60ux67b6ux6784}{%
\subsection{一、A-B-M自主学习架构}\label{ux4e00a-b-mux81eaux4e3bux5b66ux4e60ux67b6ux6784}}
...
\tightlist
\item
  系统通过前向计算和反向传播，学习一个带有参数 \(\theta\) 的表征网络 \(z=f_\theta(x)\)，以此来最小化特定的损失函数 \(\mathcal{L}\)。优化目标严格表述为：
\end{enumerate}

\[
\theta^* = \arg\min_\theta \mathbb{E}_{x\sim D}\mathcal{L}\left(f_\theta(x_{\mathrm{in}}),x_{\mathrm{tar}}\right)
\]
...
\item
  ...
  \item
    系统观察世界状态 \(s_t\)。
  \item
    利用代理策略 \(\pi\) 在世界动态模型 \(M(s_{t+1}\mid s_t,a_t)\) 下干预现实并生成动作序列 \(a_t\)。
  \item
...
\tightlist
\item
  \textbf{A 辅助 B}：现实世界的动作空间（如机器人的几百个自由度）会导致无模型强化学习产生维度爆炸。System A 可以通过提供低维度的动作/状态压缩表征、用于前向搜索的预测性世界模型，以及基于``预测误差''的内部好奇心奖励，极大降低 System B 的探索空间。
\item
  \textbf{B 辅助 A}：System A 如果仅靠静态数据容易被噪音淹没。System B 可以通过主动移动视角（主动感知）去刻意干预世界，为 System A 揭示隐藏的因果关系，并收集目标导向的高信噪比训练数据。
\end{itemize}

\textbf{System M：元控制中心（Meta-Control Plane）}

为了让机器像自然生物那样在这两种模式间流畅切换，论文引入了最核心的模块：System M。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{监控遥测数据}：System M 本身不处理高清图像或底层电机指令等高带宽数据，它仅监控低带宽的``元状态（Meta-states）''。这包括：认识论信号（预测误差、学习增益、不确定性）、物种特异性信号（对目光直视、潜在危险的硬接线反应），以及躯体信号（电量或能量水平）。
\item
  \textbf{动态数据路由}：基于固定的、通过进化硬编码而来的元策略 \(\pi(a^m\mid s^m)\)，System M 执行元动作。它能够实时切断或连接数据通路，决定系统当前是进入``探索学习模式''还是``利用推理模式''，并将特定的数据动态路由到情景记忆库（Episodic Memory）进行批量处理。
\end{enumerate}

\begin{figure}[htbp]
\centering
\includegraphics{images/image-0050.png}
\caption{System A 与 System B 交互示意图}
\end{figure}

\hypertarget{ux4e8cux53ccux5c42ux4f18ux5316}{%
\subsection{二、双层优化}\label{ux4e8cux53ccux5c42ux4f18ux5316}}
...
\[
\phi_{t+1}=\arg\min_\phi \mathcal{L}(A_0:A_K,B_0:B_K)
\]

这一优化必须受制于发育尺度的初始化和交互法则：

\[
A_0,B_0,M=Init(\phi_t)
\]
...
\hypertarget{ux4e09ux9ad8ux7ea7ux5b66ux4e60ux5f62ux6001ux5c55ux671b}{%
\subsection{三、高级学习形态展望}\label{ux4e09ux9ad8ux7ea7ux5b66ux4e60ux5f62ux6001ux5c55ux671b}}
...
\item
  \textbf{通过想象学习（Learning from Imagination）}：在休息甚至``睡眠''状态下，System M 能切断外部感官和运动输出，从情景记忆中提取经验进行高速回放，利用系统 A 在潜空间中推演长程计划，并在无需真实物理试错成本的情况下更新系统 B 的策略。
\end{itemize}
