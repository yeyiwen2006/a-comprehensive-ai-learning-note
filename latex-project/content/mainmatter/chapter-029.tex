...
问题显然存在：随着处理的 Token 越来越多（尤其是对于 128k 甚至更长的上下文），这种无衰减的累加会导致快速权重 \(W_{\mathrm{down}}\) 的数值呈线性甚至指数级增长（Unbounded growth），最终在推理时引发数值溢出或严重破坏模型原有的表征分布。

论文的解决方案是强制梯度截断（Clipping Mechanism）。为了防止状态爆炸，作者在实际的推理代码中打了一个非常暴力的``工程补丁''。在将更新量 \(\Delta W_{\mathrm{down}}^{(i)}\) 加到权重上之前，系统会计算其 Frobenius 范数 \(\|\Delta W_{\mathrm{down}}^{(i)}\|_F\)。如果该范数超过了预设的阈值 \(\tau\)，则会对其进行强制缩放：
...
在他们 4B 模型的实验中，这个极其严格的阈值被设定为 \(\tau=1e-5\)。通过这种极小阈值的强行截断，论文在维持了内积计算的高吞吐量的同时，人为压制了数值爆炸的风险，以此替代了真正数学意义上 Delta Rule 的自稳定机制。
...
其中 \(a_t=\eta_t\)，\(b_t=u'_t\)。若以二次损失为例，记忆矩阵也可写成同类线性变换。假设损失函数是二次的：

\[
l(\mathcal{M};x_t)=\|\mathcal{M}k_t-v_t\|^2
\]

根据梯度下降，更新公式为：

\[
\mathcal{M}_t=\mathcal{M}_{t-1}-\eta\nabla l(\mathcal{M}_{t-1})
\]

其中梯度为：

\[
\nabla l = 2(\mathcal{M}_{t-1}k_t-v_t)k_t^\top
\]
...
\mathcal{M}_{t-1}
-
\eta\cdot 2(\mathcal{M}_{t-1}k_t-v_t)k_t^\top
\]
...
\tightlist
\item
  \textbf{分形结构}：HOPE 模型不再区分``训练阶段''和``推理阶段''。它由多层 MLP 组成，每一层的更新速度不同：\(f_1\) 高频，可能每读一个 token 就更新一次；\(f_2\) 中频，可能每读 16 个 token 更新一次；\(f_k\) 低频，可能每读 1M 个 token 更新一次，接近传统的预训练。
\item
  \textbf{数学意义}：公式 (31) 定义了一个多时间尺度的在线梯度下降。每个 MLP 都在压缩属于它那个时间窗口内的上下文信息。这模仿了人类大脑：只有极其重要或重复的信息（通过慢速层）才会变成长期记忆，而原本的信息流（快速层）会被快速处理并遗忘。
\end{itemize}
