...
\hypertarget{ux81eaux56deux5f52ux6269ux6563ux751fux6210ux7684ux7ed3ux5408ux53caux5176transformerux67b6ux6784}{%
\section{自回归、扩散生成的结合及其Transformer架构}\label{ux81eaux56deux5f52ux6269ux6563ux751fux6210ux7684ux7ed3ux5408ux53caux5176transformerux67b6ux6784}}
...
\hypertarget{ux4e09ux6269ux6563ux6a21ux578bux7684ux7ea7ux8054}{%
\subsubsection{（三）扩散模型的级联}\label{ux4e09ux6269ux6563ux6a21ux578bux7684ux7ea7ux8054}}
...
\hypertarget{ux4e00ux7a7aux95f4ux6ce8ux610fux529b}{%
\subsubsection{（一）空间注意力}\label{ux4e00ux7a7aux95f4ux6ce8ux610fux529b}}
...
\hypertarget{ux4e8cux65f6ux95f4ux6ce8ux610fux529b}{%
\subsubsection{（二）时间注意力}\label{ux4e8cux65f6ux95f4ux6ce8ux610fux529b}}
...
\hypertarget{ux89c6ux9891ux751fux6210ux6a21ux578bux7684ux4e3bux8981ux8badux7ec3ux9636ux6bb5}{%
\section{视频生成模型的主要训练阶段}\label{ux89c6ux9891ux751fux6210ux6a21ux578bux7684ux4e3bux8981ux8badux7ec3ux9636ux6bb5}}
...
\hypertarget{ux4e8cself-forcingux81eaux5c55ux5f00ux8badux7ec3}{%
\subsubsection{（二）Self-Forcing（自展开训练）}\label{ux4e8cself-forcingux81eaux5c55ux5f00ux8badux7ec3}}
...
\hypertarget{ux4e8cabot-physworld-ux4e2dux5229ux7528ux504fux597dux4f18ux5316ux5b66ux4e60ux7269ux7406ux89c4ux5f8b}{%
\subsubsection{（二）ABot-PhysWorld 中利用偏好优化学习物理规律}\label{ux4e8cabot-physworld-ux4e2dux5229ux7528ux504fux597dux4f18ux5316ux5b66ux4e60ux7269ux7406ux89c4ux5f8b}}

高德的 ABot-PhysWorld 通过两个核心组件，把优化目标从像素相似度转向物理一致性：

Proposer module：负责根据当前任务，列一份物理规则清单，说清哪些能做，哪些绝对不行。

Scorer module：对模型生成的多个结果逐帧打分。

然后用 Diffusion-DPO 基于偏好数据提高物理合规性：物理正确的结果作为偏好样本，物理错误的结果作为非偏好样本。反复优化后，模型逐渐学会``什么动作不违反物理''，例如能够根据输入的末端位姿和夹爪状态，推演出未来的时空动力学变化，不再只是像素层面的``看起来像''。
...
\hypertarget{ux89c6ux9891ux751fux6210ux6a21ux578bux7684ux8868ux5f81ux5b66ux4e60ux548ctokenizer}{%
\section{视频生成模型的表征学习和Tokenizer}\label{ux89c6ux9891ux751fux6210ux6a21ux578bux7684ux8868ux5f81ux5b66ux4e60ux548ctokenizer}}
...
\subsubsection{（二）核心架构}\label{ux4e8cux6838ux5fc3ux67b6ux6784-1}}
...
\hypertarget{ux89c6ux9891ux751fux6210ux6269ux6563ux8fc7ux7a0bux4e2dux9690ux542bux7684ux63a8ux7406}{%
\section{视频生成扩散过程中隐含的推理}\label{ux89c6ux9891ux751fux6210ux6269ux6563ux8fc7ux7a0bux4e2dux9690ux542bux7684ux63a8ux7406}}

\hypertarget{ux4e00ux6269ux6563ux8fc7ux7a0bux5b9eux73b0ux63a8ux7406ux7684ux539fux7406}{%
\subsection{一、扩散过程实现推理的原理}\label{ux4e00ux6269ux6563ux8fc7ux7a0bux5b9eux73b0ux63a8ux7406ux7684ux539fux7406}}

此前，业界普遍假设视频模型的推理遵循``帧思维链''（Chain-of-Frames, CoF），即推理是随着视频帧的播放按时间顺序展开的。然而，本研究提出并验证了\textbf{``步思维链''（Chain-of-Steps, CoS）}机制。由于扩散模型中的双向注意力机制可以覆盖整个视频序列，因此模型实际上是在每一个去噪步（Denoising Step）中，同时对所有帧进行全局推理和不断修正。

\hypertarget{ux5de5ux4f5cux539fux7406ux63a8ux5bfc}{%
\subsubsection{工作原理推导}\label{ux5de5ux4f5cux539fux7406ux63a8ux5bfc}}
...
\hypertarget{ux591aux8defux5f84ux63a2ux7d22}{%
\subsubsection{1. 多路径探索}\label{ux591aux8defux5f84ux63a2ux7d22}}
...
\hypertarget{ux53e0ux52a0ux5f0fux63a2ux7d22}{%
\subsubsection{2. 叠加式探索}\label{ux53e0ux52a0ux5f0fux63a2ux7d22}}
...
\hypertarget{ux4e8cux63a8ux7406ux80fdux529bux53caux5176ux5728ux67b6ux6784ux4e2dux7684ux5206ux5316}{%
\subsection{二、推理能力及其在架构中的分化}\label{ux4e8cux63a8ux7406ux80fdux529bux53caux5176ux5728ux67b6ux6784ux4e2dux7684ux5206ux5316}}
...
\tightlist
\item
  \textbf{工作记忆（Working Memory）}：在遮挡或物体移出画面的任务中，早期的扩散步能够建立持久的``记忆锚点''，确保被遮挡的物体在后续帧中仍然能保持物理状态的连贯性，从而解决``客体永久性''问题。
\item
  \textbf{自我纠正与增强（Self-correction and Enhancement）}：模型在扩散过程中会出现``顿悟时刻''（Aha moments）。如果它在早期生成了错误的形状或不完整的弹道轨迹，模型有能力在经历几个扩散步后推翻之前的假设，并将其修正为全面、结构正确的最终答案。
\item
  \textbf{先感知后行动（Perception before Action）}：模型自动发展出了一套处理视频推理的通用协议。在早期的去噪步骤中，它优先做语义定位，找到 Prompt 描述的关键物体；而在后期的步骤中，才开始为其添加动作规划、运动学变化和复杂的物理交互。
\end{itemize}
...
\tightlist
\item
  \textbf{浅层（Layers 0--9）}：主要负责感知，注意力分散在全局结构和环境背景上。
\item
  \textbf{中层（约 Layer 9 至 30）}：激活特征开始高度集中到语义相关的前景物体上，并爆发出大量与运动和交互相关的推理特征。作者进行了一项关键的\textbf{因果性隐空间特征替换实验（Latent Swapping）}。当把第 20 层的隐变量替换为另一个物体的表征时，最终的逻辑推理结果发生了彻底反转。这确凿地证明了中间层是逻辑推理的核心区。
\item
  \textbf{深层（晚期层）}：将推理完成的隐式表示加以巩固和融合，生成最终所需的像素级状态。
\end{itemize}

\hypertarget{ux4e09ux63a8ux7406ux96c6ux6210ux7b97ux6cd5}{%
\subsection{三、推理集成算法}\label{ux4e09ux63a8ux7406ux96c6ux6210ux7b97ux6cd5}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{多重初始化}：给同一个模型分配 3 个不同的随机噪声种子（Seeds），独立启动前向推理进程。
\item
  \textbf{锁定关键步骤}：在决定推理轨迹走向的最关键的第一个扩散步（\(s=0\)）拦截模型。
\item
  \textbf{提取活跃窗口}：分别提取 3 个进程中 DiT 骨干网络里的``推理活跃层''，即第 20 层到第 29 层的中间层区段，其隐式表示记为 \(U^{(i)}\)。
\item
  \textbf{时空集成投票}：对这三个版本的隐变量进行空间和时间维度的平均（Averaging）。这相当于在隐空间进行了一次``专家投票''，有效过滤掉了随机种子带来的特定噪音干扰。
\item
  \textbf{引导生成}：利用平均后的稳定潜变量继续后续的生成。实验证明，这一简单的集成策略使得模型在 VBVR-Bench 推理基准测试上的绝对得分直接提升了 2\%。
\end{enumerate}
...
\tightlist
\item
  Wang, R., Cai, Z., Pu, F., et al.~(2026). \href{https://arxiv.org/abs/2603.16870}{Demystifying Video Reasoning}. arXiv:2603.16870.
\item
...
\item
  Wiedemer, T., Li, Y., Vicol, P., et al.~(2025). \href{https://arxiv.org/abs/2509.20328}{Video Models are Zero-Shot Learners and Reasoners}. arXiv:2509.20328.
\item
  Wang, M., Wang, R., Lin, J., et al.~(2026). \href{https://arxiv.org/abs/2602.20159}{A Very Big Video Reasoning Suite}. arXiv:2602.20159.
\end{itemize}
