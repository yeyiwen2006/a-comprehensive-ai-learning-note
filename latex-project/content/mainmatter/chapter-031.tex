...
\tightlist
\item
  构建内部状态，将环境观测与当前变量组合为 \(\tilde{s}_t^\tau=(o_t,A_t^\tau)\)。
\item
  网络推理：将 \(A_t^\tau\) 输入VLA骨干网络（如Transformer），预测当前速度场 \(v_\theta\)，进而计算期望去噪方向 \(\mu_{\tau+\delta}\)。
\item
  噪声评估：辅助的可学习噪声网络根据当前状态输出标准差 \(\sigma_\tau\)。
\item
  采样与记录：采样随机噪声 \(\varepsilon\)，计算下一步隐变量 \(A_t^{\tau+\delta}=\mu_\tau+\sigma_\tau\varepsilon\)；同时，将该步的对数似然 \(\log p_\theta(A_t^{\tau+\delta}\mid A_t^\tau,o_t)\) 记录到缓存中。
\end{enumerate}
...
\item
  Chen, K., Liu, Z., Zhang, T., et al.~(2025). \href{https://arxiv.org/abs/2510.25889}{\(\pi_{\mathrm{RL}}\): Online RL Fine-tuning for Flow-based Vision-Language-Action Models}. arXiv:2510.25889.
\item
