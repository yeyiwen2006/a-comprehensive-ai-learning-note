...
人类互联网的数据在规模、模态、内容范围上都是受限的，依赖人类把数据放进电脑里、``喂''给模型终究是不可持续的。我们希望具身智能将能像人类一样，主动与现实世界交互、从中获取数据并学习，实现真正意义上的自主学习。具身智能可以像LLM探索代码库一样主动探索各种环境，在交互中获取环境的各种类型的数据（如在日常生活环境中主要是视觉、听觉、触觉和传感器数据等），构建和修正环境模型、主动适应环境、主动验证自己的判断，而不是被动接收已有数据。这或许是未来实现能在任意新环境中自主探索、适应和学习规律，并在其中出色地执行新任务的智能的重要基础。
