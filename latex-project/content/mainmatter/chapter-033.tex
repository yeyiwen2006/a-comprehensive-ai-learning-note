...
\def\labelenumi{\arabic{enumi}.}
\item
  加速实验进程：具身智能可以7*24小时运行和大规模并行，避免人类操作成为时间瓶颈，缩短实验需要的总时间。在单位时间内，获取更多数据意味着能训练更好的模型乃至更好地进行模拟，从而优化得到更优的策略，进一步指导更精准地获取需要的数据，当获取数据效率高的时候这个飞轮就可以跑起来，加速整体发展。
\item
...
\item
  基于物理认知和隐性知识推动发现：有观点认为，实验技术或测量工具的发明需要对物理规律的深刻理解，以及实验室里那些难以用语言表达、不会写入文献的隐性知识。具身智能既有着对物理世界变化规律和空间结构的认知，又可以在大量实验中积累经验、理解实验的难点和痛点，解决了过去实验科学家和工程师的沟通难题，``知道什么值得做''的创造性科学直觉也可能与浸泡在实验室中的具身经验和隐性知识有关。
\end{enumerate}
