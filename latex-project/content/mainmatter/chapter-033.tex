...
\hypertarget{ux4e8cux5b9eux73b0ux81eaux4e3bux5b66ux4e60ux4e0eux81eaux6211ux6539ux8fdb}{%
\subsubsection{（二）实现自主学习与自我改进}\label{ux4e8cux5b9eux73b0ux81eaux4e3bux5b66ux4e60ux4e0eux81eaux6211ux6539ux8fdb}}
...
\item
  ...
  人类互联网的数据在规模、模态、内容范围上都是受限的，依赖人类把数据放进电脑里、``喂''给模型可能终究是不可持续的。自主学习即智能体主动在与环境的交互中获取反馈数据，并以此更新自身认知（这两步也正好和《The Bitter Lesson》中提到的搜索与学习呼应）。具身智能正可以与现实世界交互，在交互中获取所需要的各种类型的环境数据（如在日常生活环境中主要是视觉、听觉、触觉和传感器数据等）以及验证自己的判断，理论上这样可以获得可无限扩展的数据，以及将RL的可验证范围拓宽到现实世界，再结合模型层面利用这些给定数据的方法（如AI for AI等）即可实现自主学习的闭环。未来，对于具备具身能力的基础模型，或许我们可以用类似于RL的方法（如在需要探索环境的任务上训练）或其他方法训练出可以在新环境中主动学习并快速适应，进而在其中出色地执行新任务的模型。
\item
  实现具身智能的自我改进
