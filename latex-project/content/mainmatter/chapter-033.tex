...
\item
  ...
  人类互联网的数据在规模、模态、内容范围上都是受限的，依赖人类把数据放进电脑里、``喂''给模型可能终究是不可持续的，这或许意味着未来智能进一步的发展还是需要自主学习。自主学习即智能体主动探索环境、与现实世界交互，在交互中获取所需要的各种类型的环境数据（如在日常生活环境中主要是视觉、听觉、触觉和传感器数据等）并验证自己的判断，进而更新自身的认知和策略。具身智能可以让这个过程构成完整的闭环，进而可能在未来可以让这种能力通过对具备具身能力的基础模型用类似于RL的方法（如在需要探索环境的任务上训练）或其他方法训练出来。

  未来，我们希望人工智能可以在各种复杂的甚至是人类难以触及的新环境中，自主探索、主动学习并快速适应，进而在其中出色地执行新任务。实现可以主动从现实世界中获取数据的具身智能正是其中重要的一步。
\item
  实现具身智能的自我改进
