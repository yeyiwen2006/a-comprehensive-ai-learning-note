...
\begin{figure}[htbp]
\centering
\includegraphics{images/image-0013.png}
...
A3C 是经典的异步 Actor-Critic 算法；OpenAI Baselines 随后给出了同步版本 A2C，并报告其在相同硬件和超参数条件下可取得与 A3C 相当的表现。因此，二者的优劣取决于实现、硬件和任务，不能笼统地断言 A3C 一定不如 A2C。
...
一种针对持续时间偏差的修正方法是引入时间项 \(\Delta t_k\)（动作 \(k\) 的执行时长）作为权重乘数。修正后的梯度估计 \(\hat{g}\) 为：
