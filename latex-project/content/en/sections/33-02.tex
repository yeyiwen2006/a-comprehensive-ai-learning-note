...
\hypertarget{unified-worldaction-generation-models}{%
\section{Unified World--Action Generation Models}\label{unified-worldaction-generation-models}}

\Needspace{5\baselineskip}
\hypertarget{i.-from-lingbot-va-to-lingbot-va-2.0-predict-states-first-then-obtain-actions}{%
\subsection{I. From LingBot-VA to LingBot-VA 2.0: Predict States First, Then Obtain Actions}\label{i.-from-lingbot-va-to-lingbot-va-2.0-predict-states-first-then-obtain-actions}}

\Needspace{5\baselineskip}
\hypertarget{model-architecture-2}{%
\subsubsection{(1) Model Architecture}\label{model-architecture-2}}
...
\tightlist
\item
  Sequential architecture and MoT
\end{enumerate}
...
\def\labelenumi{(\arabic{enumi})}
\item
  Parameters: In LingBot-VA 2.0, for example, the action expert has fewer parameters, while the visual expert uses a relatively sparse MoE architecture, activating only the top eight of 128 experts during inference.
\item
  Hidden dimensions: In FFN/MoE layers, video has a larger hidden dimension than actions because it contains more information. To align hidden dimensions in attention layers, downsampling or upsampling is generally applied during forward propagation between layers.
\end{enumerate}
...
\caption{LingBot-VA 2.0 semantic visual--action tokenizer architecture}
\end{figure}

\begin{enumerate}
\def\labelenumi{(\arabic{enumi})}
\item
  Visual reconstruction: A reconstruction loss similar to that used in VAE training.
\item
  Semantic alignment: Freeze a perception encoder as the teacher policy and align generated visual features with it, elevating the tokenizer's view of images from low-level pixels to high-level semantic concepts.
\item
  Latent-action extraction: Train an inverse dynamics model (IDM) and forward dynamics model (FDM). The former extracts action features between two frames from their state features; the latter attempts to reconstruct the later frame's state features from the earlier frame and those action features. This loss ensures that tokenizer latents reflect action dynamics rather than merely compressing pixel features.
\end{enumerate}

\Needspace{5\baselineskip}
\hypertarget{training-data-lingbot-va-2.0}{%
\subsubsection{(2) Training Data (LingBot-VA 2.0)}\label{training-data-lingbot-va-2.0}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  General images, text, and videos: Provide the broadest appearance and dynamics priors.
\item
  Robot data: Teleoperation data, AgiBot, OXE (including DROID), and others.
\item
  First-person human videos: 65.4k episodes covering more than 3,000 scene--task combinations, with every frame annotated with both hands' 6-DoF poses and 22 finger-joint angles per hand. All human hands are aligned to parallel grippers.
\item
  Synthetic data for subsequent ICL: A multimodal LLM first analyzes tasks and generates image-editing instructions, converting the first robot frame into an initial observation of human manipulation. A video-generation model then synthesizes human-manipulation videos. Finally, a VLM scores the robot data and generated human videos for task-semantic fidelity and physical plausibility; only qualified videos are paired with the original robot videos. This covers more than ten robot-video sources, over 5,000 tasks, and more than 50,000 human--robot pairs.
\end{enumerate}

\Needspace{5\baselineskip}
\hypertarget{pretraining-tasks-lingbot-va-2.0}{%
\subsubsection{(3) Pretraining Tasks (LingBot-VA 2.0)}\label{pretraining-tasks-lingbot-va-2.0}}
...
\Needspace{5\baselineskip}
\hypertarget{asynchronous-inference}{%
\subsubsection{(4) Asynchronous Inference}\label{asynchronous-inference}}
...
\caption{LingBot-VA 2.0 asynchronous inference timeline}
\end{figure}
...
\Needspace{5\baselineskip}
\hypertarget{ii.-unified-autoregressive-prediction-of-physical-tokens}{%
\subsection{II. Unified Autoregressive Prediction of Physical Tokens}\label{ii.-unified-autoregressive-prediction-of-physical-tokens}}
...
\Needspace{5\baselineskip}
\hypertarget{core-idea-7}{%
\subsubsection{(1) Core Idea}\label{core-idea-7}}
...
\caption{Autoregressive architecture with unified physical tokens}
\end{figure}

\Needspace{5\baselineskip}
\hypertarget{autoregressive-transformer}{%
\subsubsection{(2) Autoregressive Transformer}\label{autoregressive-transformer}}
...
\tightlist
\item
  Within a frame: Block-level full attention
\end{enumerate}
...
\tightlist
\item
  Within actions: Temporal causal attention
\end{enumerate}
...
\tightlist
\item
  Actions attend unidirectionally to vision
\end{enumerate}
...
\tightlist
\item
  Across time steps: Global temporal causality
\end{enumerate}
...
\Needspace{5\baselineskip}
\hypertarget{diffusion-decoder}{%
\subsubsection{(3) Diffusion Decoder}\label{diffusion-decoder}}
...
\Needspace{5\baselineskip}
\hypertarget{boa-token}{%
\subsubsection{(4) BOA Token}\label{boa-token}}
...
\Needspace{5\baselineskip}
\hypertarget{training-workflow-2}{%
\subsubsection{(5) Training Workflow}\label{training-workflow-2}}
...
\FloatBarrier
\Needspace{5\baselineskip}
\hypertarget{references-159}{%
\subsection{References}\label{references-159}}
