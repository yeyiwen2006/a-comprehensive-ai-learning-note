...
\hypertarget{worldaction-models-without-video-generation-at-inference}{%
\section{World--Action Models without Video Generation at Inference}\label{worldaction-models-without-video-generation-at-inference}}

\Needspace{5\baselineskip}
\hypertarget{i.-background-9}{%
\subsection{I. Background}\label{i.-background-9}}
...
\Needspace{5\baselineskip}
\hypertarget{ii.-fast-wam-discarding-the-video-branch-at-inference-and-using-only-the-action-branch}{%
\subsection{II. Fast-WAM: Discarding the Video Branch at Inference and Using Only the Action Branch}\label{ii.-fast-wam-discarding-the-video-branch-at-inference-and-using-only-the-action-branch}}

\Needspace{5\baselineskip}
\hypertarget{model-architecture-and-training-workflow}{%
\subsubsection{(1) Model Architecture and Training Workflow}\label{model-architecture-and-training-workflow}}
...
\textbf{Step One: Precise Token Grouping}
...
\tightlist
\item
  Visual anchor \(f_0\): Clean, unnoised latent features of the current first-frame observation.
\item
  Future video \(f_1,\ldots,f_h\): Noisy latent features of future frames, present only during training.
\item
  Action sequence \(a_1,\ldots,a_h\): Noisy features of the action chunk to generate.
\end{enumerate}
...
\textbf{Step Two: Global Text Cross-Attention Injection}
...
\textbf{Step Three: Structured Self-Attention Masks}
...
\Needspace{5\baselineskip}
\hypertarget{inference-workflow-2}{%
\subsubsection{(2) Inference Workflow}\label{inference-workflow-2}}
...
\tightlist
\item
  Single forward pass: Retain only clean first-frame latent features and pass them once, unidirectionally, through the video backbone to extract a physically meaningful latent world representation \(z(o,l)\).
\item
\Needspace{5\baselineskip}
  Direct action denoising: The action-expert DiT directly parameterizes the action distribution from this representation:
\end{enumerate}
...
\tightlist
\item
  Output and execution: Without instantiating and denoising future frames, action sequences require only ten denoising steps with classifier-free guidance set to CFG=1.0.
\end{enumerate}

\Needspace{5\baselineskip}
\hypertarget{experimental-results}{%
\subsubsection{(3) Experimental Results}\label{experimental-results}}
...
\Needspace{5\baselineskip}
\hypertarget{iii.-dyna-2-the-video-branch-provides-visualtext-features-to-the-action-branch-without-prediction-at-inference}{%
\subsection{III. Dyna-2: The Video Branch Provides Visual--Text Features to the Action Branch without Prediction at Inference}\label{iii.-dyna-2-the-video-branch-provides-visualtext-features-to-the-action-branch-without-prediction-at-inference}}
...
\Needspace{5\baselineskip}
\hypertarget{model-architecture-3}{%
\subsubsection{(1) Model Architecture}\label{model-architecture-3}}
...
Inputs: Observed frames Vc=\{v\_1,v\_2,\ldots,v\_N\}; noisy future frames z\_t=\{z\_N+1,\ldots,z\_N+H\}; text instruction T.

Attention: Frames use causal attention. Within Vc, v\_i can attend only to v\_1,\ldots,v\_i-1; z\_t attends unidirectionally to Vc. Frames cross-attend to text, using video as queries and text as keys and values, so video vectors absorb textual information.
...
Inputs: Noisy action chunk a\_t=\{a\_N+1,\ldots,a\_N+H\}; robot proprioceptive quantities (such as current joint positions, joint velocities, gripper state, and end-effector state); observed visual features Vc' from Video Transformer layers.
...
\Needspace{5\baselineskip}
\hypertarget{iv.-being-h0.7-aligning-action-prediction-with-action-prediction-based-on-future-states-during-training}{%
\subsection{IV. Being-H0.7: Aligning ``Action Prediction'' with ``Action Prediction Based on Future States'' during Training}\label{iv.-being-h0.7-aligning-action-prediction-with-action-prediction-based-on-future-states-during-training}}

\Needspace{5\baselineskip}
\hypertarget{core-idea-8}{%
\subsubsection{(1) Core Idea}\label{core-idea-8}}
...
\Needspace{5\baselineskip}
\hypertarget{training-workflow-3}{%
\subsubsection{(2) Training Workflow}\label{training-workflow-3}}

\textbf{Step 1: Context Encoding and Sequence Construction}
...
\textbf{Step 2: Future-Feature Extraction for the Posterior Branch}
...
\textbf{Step 3: Dual-Branch Forward Pass with a Mixture of Transformers}
...
\tightlist
\item
  Action flow-matching loss: Under the diffusion-generation paradigm, compute each branch's denoising velocity-field loss for noisy actions \(\tilde a_t\). If the true action is \(a\), noising time is \(t\), and target velocity is \(u_t=a-\tilde a_t\), then:
\end{enumerate}
...
\item
\Needspace{5\baselineskip}
  Joint hidden-state alignment loss: At aligned Transformer layer \(l\), extract prior hidden state \(h_l^{\mathrm{prior}}\) and posterior hidden state \(h_l^{\mathrm{post}}\) and compute MSE:
\end{enumerate}
...
\item
\Needspace{5\baselineskip}
  Anti-collapse regularization: Penalize norm and rank to prevent alignment from driving hidden features toward all zeros or a single direction. Norm regularization is:
\end{enumerate}
...
\Needspace{5\baselineskip}
\hypertarget{inference-workflow-3}{%
\subsubsection{(3) Inference Workflow}\label{inference-workflow-3}}
...
\tightlist
\item
  Observation collection: Robot cameras capture the current scene in real time, processing and concatenating it into a context vector.
\item
  Pure prior-branch execution: Discard the large posterior branch and all future-video inputs, retaining only the Transformer backbone, latent queries \(Q\), and flow-matching generation head for diffusion.
\item
...
\FloatBarrier
\Needspace{5\baselineskip}
\hypertarget{references-160}{%
\subsection{References}\label{references-160}}
