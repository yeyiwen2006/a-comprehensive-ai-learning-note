...
\hypertarget{core-principles-of-attention}{%
\section{Core Principles of Attention}\label{core-principles-of-attention}}

\Needspace{5\baselineskip}
\hypertarget{i.-problems-with-traditional-sequence-processing-methods-and-inspiration-from-biological-attention}{%
\subsection{I. Problems with Traditional Sequence-Processing Methods and Inspiration from Biological Attention}\label{i.-problems-with-traditional-sequence-processing-methods-and-inspiration-from-biological-attention}}
...
\Needspace{5\baselineskip}
\hypertarget{iii.-how-attention-works-an-example}{%
\subsection{III. How Attention Works: An Example}\label{iii.-how-attention-works-an-example}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Setting the scene (input information): the environment contains five objects (five information sources). Each object has its own key (prominent features such as color and shape) and value (the content of the object itself).
\item
  Involuntary attention (without an explicit query): there is no strong ``subjective intention'' (query). The system tends to attend to information with the most prominent keys. For example, the coffee cup with the key ``red'' is much more visually salient than objects with black-and-white keys. Attention weights therefore naturally favor the coffee cup, amplifying its corresponding value.
\item
  Voluntary attention (with a query): you form the intention to ``read a book''; this is the query.
\item
  Matching: the system compares the similarity of this query (``want to read a book'') with the keys of all objects in the environment (``news'' for the newspaper, ``research'' for the paper, and ``literature'' for the book). Weight calculation: the query matches the book's key most closely. The value ``book'' therefore receives the highest attention weight.
\item
  Aggregation: using the calculated weights, the system takes a weighted sum of all values (newspaper content, paper content, book content, and so on). Because the book's weight is close to 1 and the other weights are close to 0, the output information comes almost entirely from the value ``book.''
\end{enumerate}
...
\Needspace{5\baselineskip}
\hypertarget{iv.-differences-from-a-fully-connected-layer}{%
\subsection{IV. Differences from a Fully Connected Layer}\label{iv.-differences-from-a-fully-connected-layer}}
...
\Needspace{5\baselineskip}
\hypertarget{v.-multi-head-attention}{%
\subsection{V. Multi-Head Attention}\label{v.-multi-head-attention}}
...
\Needspace{5\baselineskip}
\hypertarget{vi.-mathematical-formulation-of-a-multi-head-attention-layer-a-layer-not-a-network}{%
\subsection{VI. Mathematical Formulation of a Multi-Head Attention Layer (a Layer, Not a Network)}\label{vi.-mathematical-formulation-of-a-multi-head-attention-layer-a-layer-not-a-network}}
...
The following example (before splitting into multiple heads) also illustrates the ``feature recombination'' performed by \texttt{W\_Q}.

\Needspace{5\baselineskip}
Suppose the input matrix \texttt{X} is simplified to three tokens representing ``cat,'' ``chases,'' and ``mouse,'' with a small number of feature entries illustrated as follows:
...
\Needspace{5\baselineskip}
The first few columns of the projection matrix \texttt{W\_Q} can be understood as different query features:
...
\Needspace{5\baselineskip}
Then \texttt{Q{[}0,1{]}} can be approximated as:

\[
Q[0,1] = 0.8 \times 0.1 + 0.9 \times 0.7 + 0.7 \times 0.9 + \cdots = 0.08 + 0.63 + 0.63 + \cdots \approx 1.34
\]

In multi-head attention, converting a higher-dimensional \texttt{d\_model} input into a lower-dimensional \texttt{d\_k} output is underpinned by ``feature decomposition'': each token's feature vector (the \texttt{d\_model}-dimensional input) contains many types of features. In multi-head attention, multiplication by \texttt{W\_Q} first recombines the features, after which different aspects of the features are assigned to different heads. During this process, each head gives different levels of attention to different original features. These differences are reflected in the weights of \texttt{W\_Q}, but do not mean that the other features have no effect at all. In the subsequent attention computation, each attention head interacts with other tokens around one aspect of the features.
...
\tightlist
\item
  Step 2: computing attention weights separately for each head, \(\alpha=\mathrm{Softmax}\left(QK_i^T/\sqrt{d_k}\right)\)
\end{enumerate}
...
\tightlist
\item
  Step 3: multiplying by the values separately for each head, \(\mathrm{attn\_output}=\alpha V\)
\end{enumerate}
...
Multiplying this by \(v_j\) and summing over \(j\) gives \texttt{o\_i}, the \texttt{i}-th row vector of \texttt{attn\_output}.
...
\tightlist
\item
  Step 4: output projection after concatenating the heads, \(\mathrm{output}=\mathrm{attn\_output}W_O\)
\end{enumerate}

In attention, output projection applies a linear transformation to the combined output of multi-head attention. It integrates the lower-level features to obtain higher-level features or other desired results, combining the results of the different heads for decision-making. The shape of \texttt{attn\_output} is \texttt{{[}seq\_len,d\_model{]}}, and \(W_O\) has shape \([d_{\mathrm{model}},d_{\mathrm{model}}]\). Its \(i\)-th column specifies the proportions of the old features used in the new \(i\)-th feature. The result is a \([\mathrm{seq\_len},d_{\mathrm{model}}]\)-dimensional matrix rich in contextual representations, that is, a sequence of the same length as the input. How this is used depends on the task, as discussed later with the full Transformer architecture.
...
Now consider the first token, ``cat.'' Suppose its four feature values are \(x_1,x_2,x_3,x_4\) (the first row of \texttt{attn\_output}).
...
\subsection{References}\label{references-67}}
