...
\hypertarget{search-based-methods}{%
\section{Search-Based Methods}\label{search-based-methods}}
...
\Needspace{5\baselineskip}
\hypertarget{i.-tree-of-thoughts}{%
\subsection{I. Tree of Thoughts}\label{i.-tree-of-thoughts}}
...
\tightlist
\item
  Generation: generate multiple candidate thought branches at the current node (state). A branch represents a complete reasoning step, not a token; token-level branching would make the tree too wide, deep, and sparse for effective value evaluation. Prompts require a special token to mark the end of a step. Top-k sampling retains only a few branches, such as 5.
\end{enumerate}
...
\setcounter{enumi}{1}
\item
  Evaluation: score the potential of each thought node using the model itself or an external function to decide whether to retain or prune it.
\item
  Search strategy: breadth-first search (BFS) or depth-first search (DFS).
\end{enumerate}

\Needspace{5\baselineskip}
\hypertarget{ii.-self-consistency}{%
\subsection{II. Self-Consistency}\label{ii.-self-consistency}}
...
\Needspace{5\baselineskip}
\hypertarget{iii.-lats-a-language-agent-algorithm-incorporating-monte-carlo-tree-search}{%
\subsection{III. LATS: A Language-Agent Algorithm Incorporating Monte Carlo Tree Search}\label{iii.-lats-a-language-agent-algorithm-incorporating-monte-carlo-tree-search}}
...
\tightlist
\item
  Core principles (inference workflow)
\end{enumerate}
...
\tightlist
\item
  Stage 4: Back up values to nodes along the path
\end{enumerate}
...
\item
\Needspace{5\baselineskip}
  Update total action value (cumulative sum):
\end{enumerate}
...
\item
\Needspace{5\baselineskip}
  Calculate the average value used by UCB/PUCT in Stage 1:
\end{enumerate}
...
\tightlist
\item
  \textbf{Unchanged prior probabilities}: for any given position \(s\), the policy network's action probabilities \(P(s,a)\) are completely unchanged.
\item
  \textbf{Unchanged individual evaluations}: the value network's raw score for a given leaf, \(v=f_\theta(s_{\mathrm{leaf}})\), is also completely unchanged.
\end{itemize}
...
\tightlist
\item
  Definition of the Markov state
\end{enumerate}
...
\tightlist
\item
  Expanding Monte Carlo tree search
\end{enumerate}
...
\tightlist
\item
  \textbf{Root (depth \(d=0\))}: the actual current board, with ``me'' to move.
\item
  \textbf{Child (depth \(d=1\))}: the board after ``I'' hypothetically make a move, with the ``opponent'' to move.
\item
  \textbf{Grandchild (depth \(d=2\))}: the board after the ``opponent'' also moves, with ``me'' to move again.
\end{itemize}
...
\tightlist
\item
  \textbf{The current node is my turn}: PUCT seeks the action maximizing ``my'' \(Q\), while using \(\mathbf{p}\) and \(N\) to explore unknown possibilities.
\item
  \textbf{The current node is the opponent's turn}: PUCT still maximizes \(Q\), but \(Q\) is now calculated from the opponent's perspective, effectively simulating that ``the opponent will choose the action most beneficial to them and most dangerous to me.''
\end{itemize}
...
\Needspace{5\baselineskip}
\hypertarget{iv.-retrieval-of-relevant-reasoning}{%
\subsection{IV. Retrieval of Relevant Reasoning}\label{iv.-retrieval-of-relevant-reasoning}}
...
\FloatBarrier
\Needspace{5\baselineskip}
\hypertarget{references-120}{%
\subsection{References}\label{references-120}}
