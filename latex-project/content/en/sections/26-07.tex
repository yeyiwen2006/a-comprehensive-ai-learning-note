...
\hypertarget{reinforcement-learning-in-diffusion-models-and-dpo-for-diffusion}{%
\section{Reinforcement Learning in Diffusion Models and DPO for Diffusion}\label{reinforcement-learning-in-diffusion-models-and-dpo-for-diffusion}}

\Needspace{5\baselineskip}
\hypertarget{i.-difficulties-of-rl-in-diffusion-models}{%
\subsection{I. Difficulties of RL in Diffusion Models}\label{i.-difficulties-of-rl-in-diffusion-models}}

\Needspace{5\baselineskip}
\hypertarget{credit-assignment-over-long-trajectories-with-sparse-rewards}{%
\subsubsection{1. Credit assignment over long trajectories with sparse rewards}\label{credit-assignment-over-long-trajectories-with-sparse-rewards}}

\begin{itemize}
\tightlist
\item
  \textbf{The difficulty}: RL reward signals, such as aesthetic scores and human-preference alignment scores \(R(x_0)\), are extremely sparse and can only be obtained once generation has finished and the final complete image is available. With no immediate, dense intermediate rewards, a low score for the final image makes it very difficult for the RL algorithm to identify the step at which the neural network output went wrong: for example, the coarse-outline stage at \(t=0.9\) or the detailed-texture stage at \(t=0.1\).
\item
  \textbf{Consequence}: conventional advantage estimators, such as GAE, struggle to propagate reward gradients accurately backward through such long chains of ODE/SDE solver steps. Optimization becomes extremely inefficient, and credit may even be assigned incorrectly.
\end{itemize}

Value-network training is especially prone to collapse, which is why GRPO-based algorithms are more commonly used.

\Needspace{5\baselineskip}
\hypertarget{the-exploration-crisis-in-extremely-high-dimensional-continuous-action-spaces}{%
\subsubsection{2. The exploration crisis in extremely high-dimensional continuous action spaces}\label{the-exploration-crisis-in-extremely-high-dimensional-continuous-action-spaces}}

\begin{itemize}
\tightlist
\item
  \textbf{The difficulty}: effective random exploration in such a vast, high-dimensional continuous space is like finding a needle in a haystack. Blindly adding Gaussian exploration noise to the vector field or noise predicted at each step can easily introduce cascading errors into the entire differential-equation integration trajectory, causing the final image to collapse completely into meaningless noise.
\item
  \textbf{Consequence}: to prevent the generation trajectory from collapsing, researchers must set the exploration-noise variance extremely low. This, however, deprives the model of exploration capability and quickly traps it in local optima. It is also the underlying reason why DM+RL is highly prone to mode collapse and loss of generation diversity.
\end{itemize}

\Needspace{5\baselineskip}
\hypertarget{ii.-dpo-for-diffusion}{%
\subsection{II. DPO for Diffusion}\label{ii.-dpo-for-diffusion}}

\Needspace{5\baselineskip}
\hypertarget{mathematical-formulation}{%
\subsubsection{(1) Mathematical formulation}\label{mathematical-formulation}}
...
\subsubsection{(2) Workflow}\label{workflow}}
...
\tightlist
\item
  Build or collect a static offline preference dataset \(\mathcal{D}\). It contains many tuples \((c,x_w,x_l)\): a ``text prompt,'' a ``good image preferred by humans or AI,'' and a ``bad image rejected by humans or AI.''
\item
  Prepare a pretrained base diffusion model as the reference model \(\theta_{\mathrm{ref}}\) and freeze its weights.
\item
  Initialize a policy model \(\theta\) with exactly the same architecture, usually from \(\theta_{\mathrm{ref}}\). This is the model to be trained.
\end{enumerate}
...
\tightlist
\item
  Sampling and noise addition: sample a batch of \((c,x_w,x_l)\) from the dataset. Randomly sample a time step \(t\sim\mathcal{U}(0,T)\) and Gaussian noise \(\epsilon\sim\mathcal{N}(0,I)\).
\item
\Needspace{5\baselineskip}
  Construct intermediate states: use the forward-diffusion formulas to add noise to both the good and bad images up to time step \(t\):
\end{enumerate}
...
\tightlist
\item
  Compute reference-model errors (no gradient): feed the noisy good and bad images, together with \(t\) and \(c\), into the frozen reference model \(\theta_{\mathrm{ref}}\). Compute the mean squared errors between its noise predictions and the actual noise \(\epsilon\): \(\mathcal{L}_{\mathrm{ref}}(x_w)\) and \(\mathcal{L}_{\mathrm{ref}}(x_l)\).
\item
  Compute policy-model errors (with gradients): feed the same data into the policy model \(\theta\) being trained, and compute its prediction errors: \(\mathcal{L}_\theta(x_w)\) and \(\mathcal{L}_\theta(x_l)\).
\item
  Update through backpropagation: substitute these four scalar errors into the \(\mathcal{L}_{\mathrm{DPO}\text{-}\mathrm{Diff}}(\theta)\) formula. Use the automatic differentiation mechanism of a deep learning framework (such as PyTorch) to compute gradients with respect to \(\theta\), and update the model weights with an optimizer (such as AdamW).
\end{enumerate}

\FloatBarrier
\Needspace{5\baselineskip}
\hypertarget{references-134}{%
\subsection{References}\label{references-134}}
