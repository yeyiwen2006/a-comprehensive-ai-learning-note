...
\hypertarget{model-context-protocol}{%
\section{Model Context Protocol}\label{model-context-protocol}}
...
\Needspace{5\baselineskip}
\hypertarget{i.-background-6}{%
\subsection{I. Background}\label{i.-background-6}}
...
\Needspace{5\baselineskip}
\hypertarget{ii.-core-architecture}{%
\subsection{II. Core Architecture}\label{ii.-core-architecture}}

\begin{figure}[htbp]
\centering
\includegraphics{images/image-0035.png}
\caption{Interaction architecture of MCP hosts, clients, and servers}
\end{figure}
...
\Needspace{5\baselineskip}
\hypertarget{iii.-workflow}{%
\subsection{III. Workflow}\label{iii.-workflow}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Protocol version and optional capability discovery: in MCP version 2026-07-28, each request carries the protocol version, client identity, and capability information. The old \texttt{initialize}/\texttt{initialized} handshake and \texttt{Mcp-Session-Id} are no longer required. Clients wishing to learn the server's supported versions and capabilities before calling tools may invoke optional \texttt{server/discover}, or directly send a specific request and handle unsupported-version or method errors.
\item
  Capability listing: clients can use \texttt{tools/list}, \texttt{resources/list}, and similar methods to obtain currently exposed tools and resources, including tool names, purposes, and parameter schemas. The host provides necessary descriptions to the LLM according to its context-management strategy. For scalability, to avoid context explosion, and to preserve cache hits as much as possible, it can expose only a tool directory or meta-tools and load specific definitions as needed.
\item
  User request: the user asks the host a question, such as ``Find how many paying users were added to the database yesterday and generate a summary.''
\item
  LLM decision and instruction generation: the host sends the question and server-provided tool list to the model. The LLM determines that a database-query tool is needed and generates a tool call conforming to its schema.
\item
  Routing and execution: the host routes the instruction to the appropriate internal client, which wraps it as an MCP JSON-RPC request with the current protocol version and other required metadata and sends it to the server. The server executes the SQL query, obtains the data, and returns the results to the model-side client.
\item
  Context return and final response: the client adds the query results to the model's context. The LLM reasons from the retrieved data and generates a natural-language answer, which the host displays to the user.
\end{enumerate}

\FloatBarrier
\Needspace{5\baselineskip}
\hypertarget{references-123}{%
\subsection{References}\label{references-123}}
