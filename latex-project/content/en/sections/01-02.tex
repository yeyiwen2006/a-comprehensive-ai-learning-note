...
\Needspace{5\baselineskip}
\hypertarget{i.-neural-networks}{%
\subsection{I. Neural Networks}\label{i.-neural-networks}}
...
\Needspace{5\baselineskip}
\hypertarget{ii.-linear-regression}{%
\subsection{II. Linear Regression}\label{ii.-linear-regression}}
...
Note that x need not be a single number. More often, the output y is jointly determined by many factors, in which case x is a vector of values {[}x\_1,x\_2,\ldots,x\_n{]}. For example, if 10 ingredients jointly determine sweetness, then y=w\_1\emph{x\_1+\ldots+w\_10}x\_10+b. Alternatively, if w and x are written as a row vector and a column vector, respectively, the expression can still take the form y = wx + b.
...
\Needspace{5\baselineskip}
\hypertarget{iii.-activation-functions}{%
\subsection{III. Activation Functions}\label{iii.-activation-functions}}
...
\caption{Linear functions before the ReLU transformation}
\end{figure}
...
\caption{Piecewise linear functions after the ReLU transformation}
\end{figure}
...
\caption{Adding multiple ReLU functions produces a more complex piecewise linear function}
\end{figure}
...
\Needspace{5\baselineskip}
\hypertarget{iv.-loss-functions}{%
\subsection{IV. Loss Functions}\label{iv.-loss-functions}}
