...
\hypertarget{model-distillation}{%
\section{Model Distillation}\label{model-distillation}}

\Needspace{5\baselineskip}
\hypertarget{i.-the-core-idea}{%
\subsection{I. The Core Idea}\label{i.-the-core-idea}}
...
\Needspace{5\baselineskip}
\hypertarget{ii.-mathematical-formulation-1}{%
\subsection{II. Mathematical Formulation}\label{ii.-mathematical-formulation-1}}

\Needspace{5\baselineskip}
\hypertarget{softmax-with-temperature}{%
\subsubsection{1. Softmax with Temperature}\label{softmax-with-temperature}}
...
\subsubsection{2. Loss Function}\label{loss-function}}
...
\Needspace{5\baselineskip}
\hypertarget{mathematical-derivation-why-does-high-temperature-work}{%
\subsubsection{3. Mathematical Derivation: Why Does High Temperature Work?}\label{mathematical-derivation-why-does-high-temperature-work}}
...
The logits \texttt{{[}10,\ 5,\ 2{]}} explicitly tell the student that, although class 1 is the strongest, class 2 is much stronger than class 3 (by 3 units). This information is dark knowledge.
...
\tightlist
\item
  Direction alignment: distillation requires the logit vector \(z_s\) generated by the student and the teacher's logit vector \(z_t\) to point in the same direction. This means that the student must learn not only ``who comes first'' but also the relative ranking of ``second, third, \ldots, Nth.''
\item
...
\Needspace{5\baselineskip}
\hypertarget{iii.-different-methods-of-model-distillation}{%
\subsection{III. Different Methods of Model Distillation}\label{iii.-different-methods-of-model-distillation}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Response-based: use the teacher model's final-layer output. This is the most classical method (as described above).
\item
  Feature-based: learn not only the output but also force the student model's intermediate feature maps to fit the teacher model's intermediate layers. Additional convolutional layers are usually needed to match the feature-map dimensions.
\end{enumerate}
...
\Needspace{5\baselineskip}
\hypertarget{iv.-on-policy-distillation}{%
\subsection{IV. On-Policy Distillation}\label{iv.-on-policy-distillation}}
...
\subsection{References}\label{references-89}}
