...
\hypertarget{flow-matching-models-1}{%
\section{Flow-Matching Models}\label{flow-matching-models-1}}

\Needspace{5\baselineskip}
\hypertarget{i.-the-basic-idea-of-flow-matching-and-its-comparison-with-diffusion-models}{%
\subsection{I. The Basic Idea of Flow Matching and Its Comparison with Diffusion Models}\label{i.-the-basic-idea-of-flow-matching-and-its-comparison-with-diffusion-models}}
...
\tightlist
\item
  \textbf{Initial state}: at \(t=0\), all samples follow a simple distribution \(p_0(x)\), such as standard Gaussian noise, like a loose pile of sand.
\item
  \textbf{Target state}: at \(t=1\), the samples come together to form the complex data distribution \(p_1(x)\) we want to generate, such as a collection of beautiful images, like sand forming a castle.
\item
  \textbf{Velocity field}: denoted by \(v(t,x)\), this is a ``weather vane'' or ``force field'' at any time \(t\in[0,1]\) and position \(x\) in high-dimensional space. It indicates the direction and speed at which a particle at that position should move in the next instant.
\item
  \textbf{The role of the ordinary differential equation}: the rate of change of a particle's trajectory \(x_t\) over time (its velocity) is determined by the velocity field at its location. This is naturally expressed mathematically as an ordinary differential equation:
\end{itemize}
...
\Needspace{5\baselineskip}
\hypertarget{ii.-mathematical-principles}{%
\subsection{II. Mathematical Principles}\label{ii.-mathematical-principles}}

The core of flow matching is to define and learn this ``velocity field.''

\Needspace{5\baselineskip}
\hypertarget{a.-probability-path}{%
\subsubsection{A. Probability Path}\label{a.-probability-path}}
...
\Needspace{5\baselineskip}
\hypertarget{b.-velocity-field}{%
\subsubsection{B. Velocity Field}\label{b.-velocity-field}}
...
\Needspace{5\baselineskip}
\hypertarget{c.-flow-matching-objective}{%
\subsubsection{C. Flow-Matching Objective}\label{c.-flow-matching-objective}}
...
\Needspace{5\baselineskip}
\hypertarget{iii.-training-method-using-a-flow-matching-vla-model-as-an-example}{%
\subsection{III. Training Method (Using a Flow-Matching VLA Model as an Example)}\label{iii.-training-method-using-a-flow-matching-vla-model-as-an-example}}
...
\tightlist
\item
  \textbf{Feature extraction}: the VLM receives the current camera observation \(o_t\) and language \(l\), then performs a forward pass to generate the multimodal conditioning vector \(c\).
\item
  \textbf{Noise initialization}: sample Gaussian trajectory noise \(x_0\sim\mathcal{N}(0,I)\) of length \(H\) from a standard normal distribution.
\item
  \textbf{ODE solving}: over time steps \(t\in[0,1]\), the action generator \(v_\theta(x_t,t,c)\) uses context \(c\) to compute the gradient of the current state. A numerical integrator (such as Euler's or Heun's method) updates \(x_t\) step by step.
\item
  \textbf{Action execution}: take the ODE endpoint \(x_1\) as the predicted action chunk \(A_t\). The system uses model predictive control (MPC / receding-horizon control), executing only the first \(k\) actions before obtaining a new visual observation and starting the next round of closed-loop inference.
\end{enumerate}
...
\Needspace{5\baselineskip}
\hypertarget{iv.-applications-1}{%
\subsection{IV. Applications}\label{iv.-applications-1}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Generating high-resolution, high-quality images, with performance comparable to or better than diffusion models such as Stable Diffusion and usually faster generation (because straighter trajectories require fewer sampling steps).
\item
  Generating continuous sequences of video frames and handling high-dimensional spatiotemporal data.
\item
...
\FloatBarrier
\Needspace{5\baselineskip}
\hypertarget{references-135}{%
\subsection{References}\label{references-135}}
