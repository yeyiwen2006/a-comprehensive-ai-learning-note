...
\item
  删除 \texttt{18.4\ 高维向量的量化（论文）}；将原 \texttt{27.2\ Delta\ Attention} 迁移为 \texttt{19.9}，将原 \texttt{19.9\ 门控注意力（Gated\ Attention）（论文）} 顺延为 \texttt{19.10\ 门控注意力（Gated\ Attention）}，并按当前 Word 标题去掉 \texttt{20.2\ Engram模块}、\texttt{20.4\ Attention\ Residuals} 及 \texttt{21.1} 至 \texttt{21.4} 标题中的``（论文）''。
\item
