...
\Needspace{5\baselineskip}
\hypertarget{ux516dralph-loop}{%
\subsection{六、Ralph Loop}\label{ux516dralph-loop}}

2025年7月，软件工程师Geoffrey Huntley把Ralph描述为一种极简的循环式编程智能体工作流，其纯粹形式是一行Bash命令：\texttt{while\ :;\ do\ cat\ PROMPT.md\ \textbar{}\ claude-code\ ;\ done}。每一轮都会重新读取同一份任务提示并启动一次新的智能体进程；代码改动、测试结果和Git历史等外部状态可以保留，但上一轮的完整对话上下文不会自动延续。原始Ralph循环本身不包含``完成标志''或Stop Hook。后来的一些工具在这一思想上增加了可编程完成条件和Stop Hook：若测试未通过或未检测到完成条件，Hook会阻止流程结束并触发下一轮。介绍时应把这种后续封装与Huntley提出的一行循环区分开。
