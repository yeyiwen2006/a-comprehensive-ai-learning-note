...
\tightlist
\item
  在中英文完整目录中为第1章至第35章补充独立的章节标题，使``部分---章节---小节''层级清晰可见；同步更新双语目录生成逻辑，并增加章节标题、所属部分及小节归属的结构与完整性校验，避免后续重新生成时丢失或错位。
\item
...
\item
  将 LaTeX 正文拆分为共享节级文件，使总书和单节入口引用同一份正文；新增双语总书及336个单节 PDF 的统一构建、独立任务目录、三遍编译、失败保护、元数据检查、相对链接索引与构建报告。两本总 PDF 保留在原 \texttt{latex-project/build/} 并同名复制到 \texttt{output/pdf/complete-books/}，单节按语言和部分/章节集中到 \texttt{chinese-sections/}、\texttt{english-sections/}，索引和报告分别为 \texttt{PDF-INDEX.md}、\texttt{build-reports/}；PDF 和临时产物不上传。
\end{itemize}
...
\begin{itemize}
\item
  按最新 Word 原稿同步 \texttt{34.1\ 具身智能训练数据概述} 和 \texttt{34.2\ UMI数据} 的文字修改：\texttt{34.1} 将 UMI 数据扩展为``UMI及可穿戴设备采集数据''并加入外骨骼采集概述，\texttt{34.2} 新增通过可穿戴外骨骼采集灵巧操作数据的第四节，介绍 DexUMI 与 TwinDEX。补入 DexUMI 论文和 TwinDEX 官方项目页两项可核验来源；复核 \texttt{34.1} 的1处 DrawingML 裁剪及两篇共5张图后确认图片内容未变，继续沿用既有公开资源。同步重建 LaTeX 版本，README 正文统计更新为约37.4万字。
\item
  重绘 \texttt{1.3\ 梯度下降与反向传播} 中的两张手写示意图：梯度几何图统一了坐标轴、截点和垂足关系，使 \(O\)、\(P\) 与 \(\theta_1\) 轴共线、\(P\)、\(H\)、\(Q\) 共线，并准确标出 \(AO=2\)、\(OP=1\)、\(OQ=2\)；学习率图保留已审核的新版式。同步改写正文和 LaTeX 版，将局部切平面明确为 \(L=2-2\theta_1-\theta_2\)，更正两个偏导数与负梯度为 \(\frac{\partial L}{\partial\theta_1}=-2\)、\(\frac{\partial L}{\partial\theta_2}=-1\)和 \(-\nabla L=(2,1)\)，并补充 \(OH\perp PQ\) 及其与负梯度同向的几何说明。
\item
  固定 LaTeX 项目的 PDF 构建产物名称：三遍 XeLaTeX 命令统一使用 \texttt{a-comprehensive-ai-learning-note（位于github同名仓库）} 作为 job name，最终生成 \texttt{latex-project/build/a-comprehensive-ai-learning-note（位于github同名仓库）.pdf}；三遍编译可使干净构建环境中的目录页码与交叉引用稳定，并将该规则写入生成器维护的构建说明，避免重新生成项目后退回 \texttt{main.pdf}。
\item
  将 GitHub 到 Gitee 自动同步工作流使用的 \texttt{actions/checkout} 从 \texttt{v4} 升级到采用 Node.js 24 运行时的 \texttt{v6}，消除 Node.js 20 弃用警告，并保持完整历史与标签同步行为不变；同时将 README 中 v1.1、v2.0 的版本发布日期与``更新内容''分行展示，并同步更新 LaTeX 版本说明。
\item
...
\item
  全量复核 168 篇公开 Markdown 的 GitHub 数学与段落格式，修复 76 篇正文中的数学或段落格式问题；其中将分布在 63 篇正文及 README 中的 409 处有序列表标记改为 CommonMark 可识别的写法，并补齐标题、列表、正文和公式块之间缺失的空行。修正 \texttt{23.2\ 基于搜索的方法} 中与正文粘连的块公式定界符；修正 \texttt{15.6\ 注意力机制与梯度下降的等价性}、\texttt{21.5\ 强化微调前沿研究} 中矩阵与对齐环境中过量的反斜杠，以及 \texttt{25.2\ 模型的记忆} 中被隐藏制表符破坏的 \texttt{\textbackslash{}to}；同时处理会被 Markdown 误判为链接、强调、HTML 或正文的裸数学表达式、代码、伪代码与伪标签。增强上传前校验，使同行夹带正文的 \texttt{\$\$} 直接报错；章节数、文件数、正文字数和图片统计均未因这些格式修复而变化。
\item
  为 \texttt{20.3\ 流形假设与超连接} 补入正文开头提及的 Bengio、Courville 与 Vincent 表征学习综述论文。
\item
...
\item
  将 README 与 PDF 封面标题更新为 \texttt{A\ Comprehensive\ AI\ Learning\ Note（v2.0）}，并通过固定宽度缩放使 PDF 标题保持在一行；同时把 PDF 封面落款日期更新为 \texttt{2026年9月2日}。删除 \texttt{35.1}、\texttt{35.2} 中 Jim Fan 演讲参考文献末尾的中文说明，仅保留正式的 \texttt{{[}Video{]}} 文献条目。
\item
  按最新 Word 原稿同步 \texttt{30.1\ 具身智能模型的架构路线}：更新传统模块化、分层模型、VLA 与 WAM 四类路线的表述，补充世界模型规划作用；重写 OpenVLA、\(\pi_0\) 与 LingBot-VA 2.0 的动作空间说明，并新增从中间动作表示经 IK、运动学和传统控制模块转换到底层电机指令，以及策略层本体适配与控制层转换的区别。逐张确认原稿5张图片均无 DrawingML 裁剪且全部属于文字/公式截图，继续以 Markdown/LaTeX 转写而不新增公开图片；依据正文增补 YOLO、SLAM、SayCan、Code as Policies、PaLM-E、RT-2、WAM 综述、CrossFormer、URDF、逆运动学、阻抗控制、MPC 与 PID 等13条可核验来源。README 同步标明当前为 v2.0，完善第4至第6部分的重构说明，并将正文统计更新为约37.2万字。
\item
  按最新 Word 原稿补全 \texttt{34.4\ 数据的处理与对齐}：将第二节标题更新为``Qwen-RoboManip：数据的对齐''，新增跨机器人的80维动作/状态格式、0-1掩码、坐标系统一、上下文本体特征、模态两两对齐，以及日志与运动学正确性检查等内容；原稿不含图片，保留既有三条参考文献并同步 LaTeX/PDF 版本，同时将 README 正文字数统计更新为约37.1万字。
\item
  按最新 Word 原稿同步 \texttt{34.3\ 人类第一视角数据} 的文字更新：在视觉外观改写方法中加入 Qwen-RobotManip，并补充由人手轨迹经物理仿真计算机器人运动、再叠加虚拟机器人的处理过程，同时统一 IK 前后的空格；复核8处图片位置及4处 DrawingML 裁剪后确认可见图像内容未变，继续沿用既有公开资源。参考文献增补 Qwen-RobotManip 技术报告，并同步 LaTeX/PDF 版本。
\item
  将 \texttt{30.1\ 具身智能模型的架构路线} 的参考文献替换为用户指定的13条来源，补充 OpenPI、动作归一化文档、LingBot-VA 文档、Dyna-2、GEN-1/GEN-1.5 及操作空间与分层二次规划经典论文，并把参考文献中的 \(\pi_0\) 移出链接文字以保证 GitHub 正常渲染；同时为 \texttt{34.4\ 数据的处理与对齐} 增补具身纪元《预训练的秘密，不只在规模和多样性》，同步 LaTeX/PDF 版本。
\item
  按最新 Word 原稿完整重建 \texttt{34.3\ 人类第一视角数据}：同步人类第一视角视频的表征/世界模型信号、四类跨本体迁移方法以及 Dyna-2 百万小时 Scaling Law 实验；逐张检查8处 Word 可见图片及4处 DrawingML 裁剪，保留 EgoScale/LAPA 流程图、EgoMimic 遮罩对比和 Dyna-2 实验曲线为裁剪后公开资源，未使用自动 OCR。补全正文所有点名模型、数据集与基础方法的论文/技术报告，增补具身纪元《硅谷大火的ego数据，正改写具身数据新规则》和《我们之前利用人类视频数据的方法，可能都走错了方向》，并同步 LaTeX/PDF 与公开统计。
\item
  按最新 Word 原稿同步 \texttt{34.1\ 具身智能训练数据概述} 的文字更新：将动捕并入真机遥操作的四类示例，删除独立的动捕数据小节并重排后续编号，同时更新世界模型生成数据的局限性表述；经核对，原稿唯一图片仍沿用既有裁剪范围，未修改公开图片。另为 \texttt{33.4} 增补具身纪元《触觉需要自己的基础模型和Open-X Embodiment》，为 \texttt{34.2} 增补具身纪元《UMI是革命性具身数采新范式，但这仅仅是个开始》，并同步 LaTeX/PDF 版本；调整 LaTeX 生成器中参考文献标题与列表的间距，避免标题与首条引用在 PDF 中过于拥挤。
\item
  将根目录 \texttt{学习路径.md} 重命名为 \texttt{初学者学习路径.md}，删除原有按主题划分的多条路线，改为24节初学者建议阅读顺序，并在 README 新增 v1.0、v1.1、v2.0 版本说明；同时调整 README 与免责声明中的前沿研究说明，补充仓库包含对应 LaTeX 版本的说明，修复 PDF 目录中``第x部分''与部分标题之间缺少可见空格及标题后首段不缩进的问题，将 PDF 强调色调整为由天蓝到蓝的统一色阶，并同步更新入口链接、转换与校验脚本及 LaTeX 版本。
\item
