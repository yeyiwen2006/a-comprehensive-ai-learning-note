# 第30-39章参考文献同步清单（DOCX核验版）

本清单记录第 30-39 章公开 Markdown 已写入的参考文献。核验依据包括本地 DOCX 正文、Word 可见图片的裁剪后 OCR/人工转写线索，以及必要时检索到的公开一手来源。

写入原则：只写入能够由 DOCX 正文或可见图片线索唯一对应到论文、技术报告、项目页或官方仓库的来源；仅有短名但无法唯一核实公开来源的条目，不写入公开正文参考文献，而是在文末说明。

## 30. 扩散模型与流匹配模型的原理和架构

### 30-01 扩散模型的基本原理

- Ho, J., Jain, A., & Abbeel, P. (2020). [Denoising Diffusion Probabilistic Models](https://arxiv.org/abs/2006.11239). NeurIPS.
- Song, Y., Sohl-Dickstein, J., Kingma, D. P., Kumar, A., Ermon, S., & Poole, B. (2021). [Score-Based Generative Modeling through Stochastic Differential Equations](https://arxiv.org/abs/2011.13456). ICLR.
- Song, J., Meng, C., & Ermon, S. (2021). [Denoising Diffusion Implicit Models](https://arxiv.org/abs/2010.02502). ICLR.

### 30-02 随机梯度朗之万动力学

- Welling, M., & Teh, Y. W. (2011). [Bayesian Learning via Stochastic Gradient Langevin Dynamics](https://www.stats.ox.ac.uk/~teh/research/compstats/WelTeh2011a.pdf). ICML.
- Roberts, G. O., & Tweedie, R. L. (1996). [Exponential Convergence of Langevin Distributions and Their Discrete Approximations](https://doi.org/10.2307/3318418). Bernoulli.

### 30-03 Diffusion U-Net

- Ronneberger, O., Fischer, P., & Brox, T. (2015). [U-Net: Convolutional Networks for Biomedical Image Segmentation](https://arxiv.org/abs/1505.04597). MICCAI.
- Ho, J., Jain, A., & Abbeel, P. (2020). [Denoising Diffusion Probabilistic Models](https://arxiv.org/abs/2006.11239). NeurIPS.
- Rombach, R., Blattmann, A., Lorenz, D., Esser, P., & Ommer, B. (2022). [High-Resolution Image Synthesis with Latent Diffusion Models](https://arxiv.org/abs/2112.10752). CVPR.

### 30-04 Diffusion Transformer

- Dosovitskiy, A., Beyer, L., Kolesnikov, A., et al. (2021). [An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale](https://arxiv.org/abs/2010.11929). ICLR.
- Peebles, W., & Xie, S. (2023). [Scalable Diffusion Models with Transformers](https://arxiv.org/abs/2212.09748). ICCV.

### 30-05 扩散模型如何解决“折中问题”

- Song, J., Meng, C., & Ermon, S. (2021). [Denoising Diffusion Implicit Models](https://arxiv.org/abs/2010.02502). ICLR.
- Karras, T., Aittala, M., Aila, T., & Laine, S. (2022). [Elucidating the Design Space of Diffusion-Based Generative Models](https://arxiv.org/abs/2206.00364). NeurIPS.

### 30-06 Just Image Transformer（论文）

- Li, T., & He, K. (2025). [Back to Basics: Let Denoising Generative Models Denoise](https://arxiv.org/abs/2511.13720). arXiv:2511.13720.
- Ho, J., Jain, A., & Abbeel, P. (2020). [Denoising Diffusion Probabilistic Models](https://arxiv.org/abs/2006.11239). NeurIPS.
- Lipman, Y., Chen, R. T. Q., Ben-Hamu, H., Nickel, M., & Le, M. (2023). [Flow Matching for Generative Modeling](https://arxiv.org/abs/2210.02747). ICLR.

### 30-07 流匹配模型

- Lipman, Y., Chen, R. T. Q., Ben-Hamu, H., Nickel, M., & Le, M. (2023). [Flow Matching for Generative Modeling](https://arxiv.org/abs/2210.02747). ICLR.
- Liu, X., Gong, C., & Liu, Q. (2023). [Flow Straight and Fast: Learning to Generate and Transfer Data with Rectified Flow](https://arxiv.org/abs/2209.03003). ICLR.

### 30-08 离散流匹配（论文）

- Gat, I., Remez, T., Shaul, N., Kreuk, F., Chen, R. T. Q., Synnaeve, G., Adi, Y., & Lipman, Y. (2024). [Discrete Flow Matching](https://arxiv.org/abs/2407.15595). arXiv:2407.15595.
- Campbell, A., Yim, J., Barzilay, R., Rainforth, T., & Jaakkola, T. (2024). [Generative Flows on Discrete State-Spaces: Enabling Multimodal Flows with Applications to Protein Co-Design](https://arxiv.org/abs/2402.04997). arXiv:2402.04997.

### 30-09 从非纯噪声开始的流匹配（论文）

- Jia, J., Li, G., Chen, X., An, T., Hu, Y., Li, J., Guo, X., & Yang, J. (2026). [Action-to-Action Flow Matching](https://arxiv.org/abs/2602.07322). arXiv:2602.07322.
- Lipman, Y., Chen, R. T. Q., Ben-Hamu, H., Nickel, M., & Le, M. (2023). [Flow Matching for Generative Modeling](https://arxiv.org/abs/2210.02747). ICLR.

### 30-10 基于RL的扩散模型调参（论文）

- Black, K., Janner, M., Du, Y., Kostrikov, I., & Levine, S. (2023). [Training Diffusion Models with Reinforcement Learning](https://arxiv.org/abs/2305.13301). arXiv:2305.13301.
- Fan, Y., Watkins, O., Du, Y., Liu, H., Ryu, M., Boutilier, C., Abbeel, P., Ghavamzadeh, M., Lee, K., & Lee, K. (2023). [DPOK: Reinforcement Learning for Fine-tuning Text-to-Image Diffusion Models](https://arxiv.org/abs/2305.16381). NeurIPS.
- Wallace, B., Gokul, A., Ermon, S., & Naik, N. (2024). [Diffusion Model Alignment Using Direct Preference Optimization](https://arxiv.org/abs/2311.12908). CVPR.
- Ni, Z., Wang, Y., Hua, Y., Zhou, R., Guo, J., Song, J., Zheng, B., & Huang, G. (2026). [AdaGen: Learning Adaptive Policy for Image Synthesis](https://arxiv.org/abs/2603.06993). arXiv:2603.06993.

## 31. 扩散模型与流匹配模型的强化学习

### 31-01 扩散模型与流匹配模型中强化学习的难点

- Black, K., Janner, M., Du, Y., Kostrikov, I., & Levine, S. (2023). [Training Diffusion Models with Reinforcement Learning](https://arxiv.org/abs/2305.13301). arXiv:2305.13301.
- Lipman, Y., Chen, R. T. Q., Ben-Hamu, H., Nickel, M., & Le, M. (2023). [Flow Matching for Generative Modeling](https://arxiv.org/abs/2210.02747). ICLR.
- Wallace, B., Gokul, A., Ermon, S., & Naik, N. (2024). [Diffusion Model Alignment Using Direct Preference Optimization](https://arxiv.org/abs/2311.12908). CVPR.

### 31-02 Offline RL转化为加权监督学习（论文）

- Peters, J., & Schaal, S. (2007). [Reinforcement Learning by Reward-Weighted Regression for Operational Space Control](https://dl.acm.org/doi/10.1145/1273496.1273590). ICML.
- Peng, X. B., Kumar, A., Zhang, G., & Levine, S. (2019). [Advantage-Weighted Regression: Simple and Scalable Off-Policy Reinforcement Learning](https://arxiv.org/abs/1910.00177). arXiv:1910.00177.
- Nair, A., Dalal, M., Gupta, A., & Levine, S. (2020). [Accelerating Online Reinforcement Learning with Offline Datasets](https://arxiv.org/abs/2006.09359). arXiv:2006.09359.
- Wallace, B., Gokul, A., Ermon, S., & Naik, N. (2024). [Diffusion Model Alignment Using Direct Preference Optimization](https://arxiv.org/abs/2311.12908). CVPR.

### 31-03 每步引入可学习噪声的Online RL（论文）

- Chen, K., Liu, Z., Zhang, T., Guo, Z., Xu, S., Lin, H., Zang, H., Li, X., Zhang, Q., Yu, Z., Fan, G., Huang, T., Wang, Y., & Yu, C. (2025). [π_RL: Online RL Fine-tuning for Flow-based Vision-Language-Action Models](https://arxiv.org/abs/2510.25889). arXiv:2510.25889.
- Schulman, J., Wolski, F., Dhariwal, P., Radford, A., & Klimov, O. (2017). [Proximal Policy Optimization Algorithms](https://arxiv.org/abs/1707.06347). arXiv:1707.06347.

### 31-04 FlowGRPO：化ODE为SDE（论文）

- Liu, J., Liu, G., Liang, J., et al. (2025). [Flow-GRPO: Training Flow Matching Models via Online RL](https://arxiv.org/abs/2505.05470). arXiv:2505.05470.
- Shao, Z., Wang, P., Zhu, Q., et al. (2024). [DeepSeekMath: Pushing the Limits of Mathematical Reasoning in Open Language Models](https://arxiv.org/abs/2402.03300). arXiv:2402.03300.

## 32. 具身智能的基本知识

### 32-01 具身智能的模型架构路线

- Brohan, A., Brown, N., Carbajal, J., et al. (2022). [RT-1: Robotics Transformer for Real-World Control at Scale](https://arxiv.org/abs/2212.06817). arXiv:2212.06817.
- Brohan, A., Brown, N., Carbajal, J., et al. (2023). [RT-2: Vision-Language-Action Models Transfer Web Knowledge to Robotic Control](https://arxiv.org/abs/2307.15818). arXiv:2307.15818.
- Kim, M. J., Pertsch, K., Karamcheti, S., et al. (2024). [OpenVLA: An Open-Source Vision-Language-Action Model](https://arxiv.org/abs/2406.09246). arXiv:2406.09246.

### 32-02 具身智能的训练数据

- O'Neill, A., Rehman, A., Gupta, A., et al. (2024). [Open X-Embodiment: Robotic Learning Datasets and RT-X Models](https://arxiv.org/abs/2310.08864). ICRA.
- Walke, H., Black, K., Lee, A., et al. (2023). [BridgeData V2: A Dataset for Robot Learning at Scale](https://arxiv.org/abs/2308.12952). arXiv:2308.12952.

### 32-03 策略函数的训练方法

- Chi, C., Feng, S., Du, Y., Xu, Z., Cousineau, E., Burchfiel, B., & Song, S. (2023). [Diffusion Policy: Visuomotor Policy Learning via Action Diffusion](https://arxiv.org/abs/2303.04137). RSS.
- Zhao, T. Z., Kumar, V., Levine, S., & Finn, C. (2023). [Learning Fine-Grained Bimanual Manipulation with Low-Cost Hardware](https://arxiv.org/abs/2304.13705). RSS.

### 32-04 VLA模型

- Brohan, A., Brown, N., Carbajal, J., et al. (2023). [RT-2: Vision-Language-Action Models Transfer Web Knowledge to Robotic Control](https://arxiv.org/abs/2307.15818). arXiv:2307.15818.
- Kim, M. J., Pertsch, K., Karamcheti, S., et al. (2024). [OpenVLA: An Open-Source Vision-Language-Action Model](https://arxiv.org/abs/2406.09246). arXiv:2406.09246.
- Black, K., Brown, N., Driess, D., et al. (2024). [π0: A Vision-Language-Action Flow Model for General Robot Control](https://arxiv.org/abs/2410.24164). arXiv:2410.24164.

## 33. 世界模型的基本知识

### 33-01 世界模型概况

- Ha, D., & Schmidhuber, J. (2018). [World Models](https://arxiv.org/abs/1803.10122). arXiv:1803.10122.
- LeCun, Y. (2022). [A Path Towards Autonomous Machine Intelligence](https://openreview.net/forum?id=BZ5a1r-kVsf). OpenReview.
- Hafner, D., Lillicrap, T., Ba, J., & Norouzi, M. (2020). [Dream to Control: Learning Behaviors by Latent Imagination](https://arxiv.org/abs/1912.01603). ICLR.

### 33-02 世界模型的主要技术路线

- Hafner, D., Pasukonis, J., Ba, J., & Lillicrap, T. (2023). [Mastering Diverse Domains through World Models](https://arxiv.org/abs/2301.04104). arXiv:2301.04104.
- Assran, M., Duval, Q., Misra, I., et al. (2023). [Self-Supervised Learning from Images with a Joint-Embedding Predictive Architecture](https://arxiv.org/abs/2301.08243). CVPR.
- Bruce, J., Dennis, M., Edwards, A., et al. (2024). [Genie: Generative Interactive Environments](https://arxiv.org/abs/2402.15391). ICML.

### 33-03 世界模型的训练数据与泛化性

- O'Neill, A., Rehman, A., Gupta, A., et al. (2024). [Open X-Embodiment: Robotic Learning Datasets and RT-X Models](https://arxiv.org/abs/2310.08864). ICRA.
- Grauman, K., Westbury, A., Byrne, E., et al. (2022). [Ego4D: Around the World in 3,000 Hours of Egocentric Video](https://arxiv.org/abs/2110.07058). CVPR.

## 34. 多模态生成与生成式世界模型

### 34-01 自回归、扩散的结合与时空Transformer

- Kondratyuk, D., Yu, L., Gu, X., et al. (2023). [VideoPoet: A Large Language Model for Zero-Shot Video Generation](https://arxiv.org/abs/2312.14125). arXiv:2312.14125.
- Villegas, R., Yang, J., Hong, S., et al. (2022). [Phenaki: Variable Length Video Generation from Open Domain Textual Description](https://arxiv.org/abs/2210.02399). arXiv:2210.02399.
- Peebles, W., & Xie, S. (2023). [Scalable Diffusion Models with Transformers](https://arxiv.org/abs/2212.09748). ICCV.
- Bruce, J., Dennis, M., Edwards, A., et al. (2024). [Genie: Generative Interactive Environments](https://arxiv.org/abs/2402.15391). ICML.

### 34-02 生成式世界模型的主要训练阶段

- Bruce, J., Dennis, M., Edwards, A., et al. (2024). [Genie: Generative Interactive Environments](https://arxiv.org/abs/2402.15391). ICML.
- Yang, M., Du, Y., Ghasemipour, S. K. S., Tompson, J., Schuurmans, D., & Abbeel, P. (2023). [GAIA-1: A Generative World Model for Autonomous Driving](https://arxiv.org/abs/2309.17080). arXiv:2309.17080.

### 34-03 扩散模型的蒸馏与Self Forcing

- Salimans, T., & Ho, J. (2022). [Progressive Distillation for Fast Sampling of Diffusion Models](https://arxiv.org/abs/2202.00512). ICLR.
- Song, Y., Dhariwal, P., Chen, M., & Sutskever, I. (2023). [Consistency Models](https://arxiv.org/abs/2303.01469). ICML.
- Yin, T., Gharbi, M., Zhang, R., Shechtman, E., Durand, F., Freeman, W. T., & Park, T. (2024). [One-step Diffusion with Distribution Matching Distillation](https://arxiv.org/abs/2311.18828). CVPR.
- Huang, X., Li, Z., He, G., Zhou, M., & Shechtman, E. (2025). [Self Forcing: Bridging the Train-Test Gap in Autoregressive Video Diffusion](https://arxiv.org/abs/2506.08009). arXiv:2506.08009.
- Savva, G., Michel, O., Lu, D., Waiwitlikhit, S., Meehan, T., Mishra, D., Poddar, S., Lu, J., & Xie, S. (2026). [Solaris: Building a Multiplayer Video World Model in Minecraft](https://arxiv.org/abs/2602.22208). arXiv:2602.22208.
- Goyal, S., Agrawal, S., Anil, G. G., Jain, P., Paul, S., & Kusupati, A. (2026). [ELT: Elastic Looped Transformers for Visual Generation](https://arxiv.org/abs/2604.09168). arXiv:2604.09168.

### 34-04 生成式世界模型中的多种扩散范式（论文）

- Austin, J., Johnson, D. D., Ho, J., Tarlow, D., & van den Berg, R. (2021). [Structured Denoising Diffusion Models in Discrete State-Spaces](https://arxiv.org/abs/2107.03006). NeurIPS.
- Hoogeboom, E., Nielsen, D., Jaini, P., Forr?, P., & Welling, M. (2021). [Argmax Flows and Multinomial Diffusion: Learning Categorical Distributions](https://arxiv.org/abs/2102.05379). NeurIPS.
- Shi, Y., De Bortoli, V., Campbell, A., & Doucet, A. (2024). [Diffusion Schrödinger Bridge Matching](https://arxiv.org/abs/2303.16852). NeurIPS.
- Hafner, D., Yan, W., & Lillicrap, T. (2025). [Training Agents Inside of Scalable World Models](https://arxiv.org/abs/2509.24527). arXiv:2509.24527.
- Shi, Q., Bai, J., Zhao, Z., Chai, W., Yu, K., Wu, J., Song, S., Tong, Y., Li, X., Li, X., & Yan, S. (2025). [Muddit: Liberating Generation Beyond Text-to-Image with a Unified Discrete Diffusion Model](https://arxiv.org/abs/2505.23606). arXiv:2505.23606.

### 34-05 Diffusion Forcing与上下文分步去噪（论文）

- Chen, B., Monso, D. M., Du, Y., Simchowitz, M., Tedrake, R., & Sitzmann, V. (2024). [Diffusion Forcing: Next-token Prediction Meets Full-Sequence Diffusion](https://arxiv.org/abs/2407.01392). NeurIPS.

### 34-06 生成式世界模型的上下文工程（论文）

- Wang, Z., Liu, Z., Li, J., Huang, K., Xu, B., Kang, F., An, M., Wang, P., Jiang, B., Wei, Y., Xietian, Y., Pei, J., Hu, L., Jiang, B., Xue, H., Wang, Z., Sun, H., Li, W., Ouyang, W., He, X., Liu, Y., Li, Y., & Yang, Y. (2026). [Matrix-Game 3.0: Real-Time and Streaming Interactive World Model with Long-Horizon Memory](https://arxiv.org/abs/2604.08995). arXiv:2604.08995.
- Chen, B., Monso, D. M., Du, Y., Simchowitz, M., Tedrake, R., & Sitzmann, V. (2024). [Diffusion Forcing: Next-token Prediction Meets Full-Sequence Diffusion](https://arxiv.org/abs/2407.01392). NeurIPS.

### 34-07 生成式世界模型的表征学习与Tokenizer（论文）

- van den Oord, A., Vinyals, O., & Kavukcuoglu, K. (2017). [Neural Discrete Representation Learning](https://arxiv.org/abs/1711.00937). NeurIPS.
- Esser, P., Rombach, R., & Ommer, B. (2021). [Taming Transformers for High-Resolution Image Synthesis](https://arxiv.org/abs/2012.09841). CVPR.
- Rombach, R., Blattmann, A., Lorenz, D., Esser, P., & Ommer, B. (2022). [High-Resolution Image Synthesis with Latent Diffusion Models](https://arxiv.org/abs/2112.10752). CVPR.

## 35. 建模三维与对象的世界模型

### 35-01 同一性三维世界的多视角世界模型（论文）

- World Labs team. (2025). [RTFM: A Real-Time Frame Model](https://www.worldlabs.ai/blog/rtfm). World Labs Research Preview.
- Sch?nberger, J. L., & Frahm, J.-M. (2016). [Structure-from-Motion Revisited](https://openaccess.thecvf.com/content_cvpr_2016/html/Schonberger_Structure-From-Motion_Revisited_CVPR_2016_paper.html). CVPR.

### 35-02 多视角一致性的具身世界模型（论文）

- Zhang, Z., Ren, H., Sun, Y., Sheng, Y., Wang, H., Wu, Z., Lin, H., Bacon, P.-L., & Yu, Y. (2026). [Towards Practical World Model-based Reinforcement Learning for Vision-Language-Action Models](https://openreview.net/forum?id=gB1yFEd106). ICLR 2026 Workshop on World Models.
- Savva, G., Michel, O., Lu, D., Waiwitlikhit, S., Meehan, T., Mishra, D., Poddar, S., Lu, J., & Xie, S. (2026). [Solaris: Building a Multiplayer Video World Model in Minecraft](https://arxiv.org/abs/2602.22208). arXiv:2602.22208.

### 35-03 以对象为中心的世界模型（论文）

- Kipf, T., van der Pol, E., & Welling, M. (2020). [Contrastive Learning of Structured World Models](https://arxiv.org/abs/1911.12247). ICLR.
- Greff, K., Kaufmann, R. L., Kabra, R., et al. (2019). [Multi-Object Representation Learning with Iterative Variational Inference](https://arxiv.org/abs/1903.00450). ICML.
- Locatello, F., Weissenborn, D., Unterthiner, T., et al. (2020). [Object-Centric Learning with Slot Attention](https://arxiv.org/abs/2006.15055). NeurIPS.
- Wu, Z., Dvornik, N., Greff, K., Kipf, T., & Garg, A. (2023). [SlotFormer: Unsupervised Visual Dynamics Simulation with Object-Centric Models](https://arxiv.org/abs/2210.05861). ICLR.

## 36. 基于隐变量预测支持下游任务的世界模型

### 36-01 RL中的世界模型

- Ha, D., & Schmidhuber, J. (2018). [World Models](https://arxiv.org/abs/1803.10122). arXiv:1803.10122.
- Hafner, D., Lillicrap, T., Fischer, I., et al. (2019). [Learning Latent Dynamics for Planning from Pixels](https://arxiv.org/abs/1811.04551). ICML.
- Hafner, D., Pasukonis, J., Ba, J., & Lillicrap, T. (2023). [Mastering Diverse Domains through World Models](https://arxiv.org/abs/2301.04104). arXiv:2301.04104.

### 36-02 RSSM与Block-Causal Transformer

- Hafner, D., Lillicrap, T., Fischer, I., et al. (2019). [Learning Latent Dynamics for Planning from Pixels](https://arxiv.org/abs/1811.04551). ICML.
- Hafner, D., Lillicrap, T., Ba, J., & Norouzi, M. (2020). [Dream to Control: Learning Behaviors by Latent Imagination](https://arxiv.org/abs/1912.01603). ICLR.
- Hafner, D., Pasukonis, J., Ba, J., & Lillicrap, T. (2023). [Mastering Diverse Domains through World Models](https://arxiv.org/abs/2301.04104). arXiv:2301.04104.
- Hafner, D., Yan, W., & Lillicrap, T. (2025). [Training Agents Inside of Scalable World Models](https://arxiv.org/abs/2509.24527). arXiv:2509.24527.

### 36-03 损失分步解耦（论文）

- Hafner, D., Pasukonis, J., Ba, J., & Lillicrap, T. (2023). [Mastering Diverse Domains through World Models](https://arxiv.org/abs/2301.04104). arXiv:2301.04104.
- Hafner, D., Yan, W., & Lillicrap, T. (2025). [Training Agents Inside of Scalable World Models](https://arxiv.org/abs/2509.24527). arXiv:2509.24527.
- Ren, Z., Wei, Y., Yu, X., Luo, G., Zhao, Y., Kang, B., Feng, J., & Jin, X. (2026). [VideoWorld 2: Learning Transferable Knowledge from Real-world Videos](https://arxiv.org/abs/2602.10102). arXiv:2602.10102.

### 36-04 基于隐变量实现多步规划（论文）

- Hafner, D., Lillicrap, T., Ba, J., & Norouzi, M. (2020). [Dream to Control: Learning Behaviors by Latent Imagination](https://arxiv.org/abs/1912.01603). ICLR.
- Hansen, N., Su, H., & Wang, X. (2022). [Temporal Difference Learning for Model Predictive Control](https://arxiv.org/abs/2203.04955). ICML.

### 36-05 任意步动力学预测（论文）

- Lin, H., Xu, Y.-Y., Sun, Y., Zhang, Z., Li, Y.-C., Jia, C., Ye, J., Zhang, J., & Yu, Y. (2025). [Any-step Dynamics Model Improves Future Predictions for Online and Offline Reinforcement Learning](https://openreview.net/forum?id=JZCxlrwjZ8). ICLR.
- Lin, H., Xiao, S., Li, Y.-C., Zhang, Z., Sun, Y., Jia, C., & Yu, Y. (2026). [ADM-v2: Pursuing Full-Horizon Roll-out in Dynamics Models for Offline Policy Learning and Evaluation](https://openreview.net/forum?id=ICbXEwqpga). ICLR.

### 36-06 Action Space的学习（论文）

- Schmidt, D., & Jiang, M. (2024). [Learning to Act without Actions](https://openreview.net/forum?id=rnnt2Be6nN). International Conference on Learning Representations.
- Garrido, Q., Nagarajan, T., Terver, B., Ballas, N., LeCun, Y., & Rabbat, M. (2026). [Learning Latent Action World Models In The Wild](https://arxiv.org/abs/2601.05230). arXiv:2601.05230.

## 37. 在抽象表示空间中预测的世界模型

### 37-01 V-JEPA（论文）

- Bardes, A., Garrido, Q., Ponce, J., et al. (2024). [V-JEPA: Revisiting Feature Prediction for Learning Visual Representations from Video](https://arxiv.org/abs/2404.08471). arXiv:2404.08471.
- Assran, M., Duval, Q., Misra, I., et al. (2023). [Self-Supervised Learning from Images with a Joint-Embedding Predictive Architecture](https://arxiv.org/abs/2301.08243). CVPR.

### 37-02 隐变量的时间直道化（论文）

- Wang, Y., Bounou, O., Zhou, G., Balestriero, R., Rudner, T. G. J., LeCun, Y., & Ren, M. (2026). [Temporal Straightening for Latent Planning](https://arxiv.org/abs/2603.12231). arXiv:2603.12231.

### 37-03 LeWorldModel（论文）

- Maes, H., Le Lidec, Q., Scieur, D., LeCun, Y., & Balestriero, R. (2026). [LeWorldModel: Stable End-to-End Joint-Embedding Predictive Architecture from Pixels](https://arxiv.org/abs/2603.19312). arXiv:2603.19312.
- Assran, M., Duval, Q., Misra, I., et al. (2023). [Self-Supervised Learning from Images with a Joint-Embedding Predictive Architecture](https://arxiv.org/abs/2301.08243). CVPR.

## 38. 结合世界模型的具身智能与物理推理

### 38-01 结合世界模型的具身智能与物理推理概述

- Ha, D., & Schmidhuber, J. (2018). [World Models](https://arxiv.org/abs/1803.10122). arXiv:1803.10122.
- LeCun, Y. (2022). [A Path Towards Autonomous Machine Intelligence](https://openreview.net/forum?id=BZ5a1r-kVsf). OpenReview.
- Battaglia, P. W., Hamrick, J. B., & Tenenbaum, J. B. (2013). [Simulation as an Engine of Physical Scene Understanding](https://www.pnas.org/doi/10.1073/pnas.1306572110). Proceedings of the National Academy of Sciences.

### 38-02 世界-策略解耦架构的具身智能与物理推理（论文）

- Schrittwieser, J., Antonoglou, I., Hubert, T., et al. (2020). [Mastering Atari, Go, Chess and Shogi by Planning with a Learned Model](https://www.nature.com/articles/s41586-020-03051-4). Nature.
- Ye, W., Liu, S., Kurutach, T., Abbeel, P., & Gao, Y. (2021). [Mastering Atari Games with Limited Data](https://arxiv.org/abs/2111.00210). NeurIPS.
- Zhang, Z., Ren, H., Sun, Y., Sheng, Y., Wang, H., Wu, Z., Lin, H., Bacon, P.-L., & Yu, Y. (2026). [Towards Practical World Model-based Reinforcement Learning for Vision-Language-Action Models](https://openreview.net/forum?id=gB1yFEd106). ICLR 2026 Workshop on World Models.

### 38-03 世界-策略顺序架构的具身智能与物理推理（论文）

- Zhang, K., Ge, K., Chi, X., Zhang, R., Shi, S., Dong, Z., Han, S., & Zhang, S. (2025). [Can World Models Benefit VLMs for World Dynamics?](https://arxiv.org/abs/2510.00855). arXiv:2510.00855.

### 38-04 世界-策略统一架构的具身智能（论文）

- Unitree Robotics. (2025). [UnifoLM-WMA-0: World-Model-Action Architecture for General Robots](https://github.com/unitreerobotics/unifolm-world-model-action). GitHub repository.
- Li, L., Zhang, Q., Yu, M., Gao, Z., Luo, Y., Xue, N., Yang, S., Zhu, X., Wang, R., Shen, Y., Han, F., & Xu, Y. (2026). [Causal World Modeling for Robot Control](https://arxiv.org/abs/2601.21998). arXiv:2601.21998.

### 38-05 仅在训练时融入预测能力的具身智能（论文）

- Yuan, T., Dong, Z., Liu, Y., & Zhao, H. (2026). [Fast-WAM: Do World Action Models Need Test-time Future Imagination?](https://arxiv.org/abs/2603.16666). arXiv:2603.16666.

### 38-06 世界模型与具身智能的测试时学习（论文）

- Jang, J., Yoo, M., Yoon, S., & Woo, H. (2026). [Test-Time Mixture of World Models for Embodied Agents in Dynamic Environments](https://openreview.net/forum?id=LQD1MrnbxH). ICLR.
- Yu, S., Zhang, Y., Wang, Z., Yoon, J., Yao, H., Ding, M., & Bansal, M. (2026). [When and How Much to Imagine: Adaptive Test-Time Scaling with World Models for Visual Spatial Reasoning](https://arxiv.org/abs/2602.08236). arXiv:2602.08236.

## 39. 统一架构的多模态理解-生成模型

### 39-01 多模态统一理解-生成模型概述

- Team Chameleon. (2024). [Chameleon: Mixed-Modal Early-Fusion Foundation Models](https://arxiv.org/abs/2405.09818). arXiv:2405.09818.
- Xie, J., Mao, W., Bai, Z., Zhang, D. J., Wang, W., Lin, K. Q., Gu, Y., Chen, Z., Yang, Z., & Shou, M. Z. (2024). [Show-o: One Single Transformer to Unify Multimodal Understanding and Generation](https://arxiv.org/abs/2408.12528). arXiv:2408.12528.
- Wu, C., Chen, X., Wu, Z., Ma, Y., Liu, X., Pan, Z., Liu, W., Xie, Z., Yu, X., Ruan, C., & Luo, P. (2024). [Janus: Decoupling Visual Encoding for Unified Multimodal Understanding and Generation](https://arxiv.org/abs/2410.13848). arXiv:2410.13848.

### 39-02 多模态统一理解-生成模型的输入编码（论文）

- Team Chameleon. (2024). [Chameleon: Mixed-Modal Early-Fusion Foundation Models](https://arxiv.org/abs/2405.09818). arXiv:2405.09818.
- Wu, C., Chen, X., Wu, Z., Ma, Y., Liu, X., Pan, Z., Liu, W., Xie, Z., Yu, X., Ruan, C., & Luo, P. (2024). [Janus: Decoupling Visual Encoding for Unified Multimodal Understanding and Generation](https://arxiv.org/abs/2410.13848). arXiv:2410.13848.
- Sun, Q., Yu, Q., Cui, Y., Zhang, F., Zhang, X., Wang, Y., Gao, H., Liu, J., Huang, T., & Wang, X. (2023). [Emu: Generative Pretraining in Multimodality](https://arxiv.org/abs/2307.05222). arXiv:2307.05222.
- FoundationVision. (2025). [UniTok: A Unified Tokenizer for Visual Generation and Understanding](https://arxiv.org/abs/2502.20321). arXiv:2502.20321.
- Meituan LongCat Team, Xiao, B., Wang, C., Li, C., Zhang, C., Peng, C., Yu, H., Yang, H., et al. (2026). [LongCat-Next: Lexicalizing Modalities as Discrete Tokens](https://arxiv.org/abs/2603.27538). arXiv:2603.27538.

### 39-03 多模态统一理解-生成模型的主干架构与训练（论文）

- Xie, J., Mao, W., Bai, Z., Zhang, D. J., Wang, W., Lin, K. Q., Gu, Y., Chen, Z., Yang, Z., & Shou, M. Z. (2024). [Show-o: One Single Transformer to Unify Multimodal Understanding and Generation](https://arxiv.org/abs/2408.12528). arXiv:2408.12528.
- Zhou, C., Yu, L., Babu, A., Tirumala, K., Yasunaga, M., Shamis, L., Kahn, J., Ma, X., Zettlemoyer, L., & Levy, O. (2024). [Transfusion: Predict the Next Token and Diffuse Images with One Multi-Modal Model](https://arxiv.org/abs/2408.11039). arXiv:2408.11039.
- Deng, C., Zhu, D., et al. (2025). [Emerging Properties in Unified Multimodal Pretraining](https://arxiv.org/abs/2505.14683). arXiv:2505.14683.
- Shi, Q., Bai, J., Zhao, Z., Chai, W., Yu, K., Wu, J., Song, S., Tong, Y., Li, X., Li, X., & Yan, S. (2025). [Muddit: Liberating Generation Beyond Text-to-Image with a Unified Discrete Diffusion Model](https://arxiv.org/abs/2505.23606). arXiv:2505.23606.

### 39-04 将视觉融入CoT的推理（论文）

- Yang, C., Lin, Z., Zhao, Y., Xiao, F., He, H., Zhao, Q., et al. (2026). [Context Unrolling in Omni Models](https://arxiv.org/abs/2604.21921). arXiv:2604.21921.
- Wu, J., Zhang, X., Yuan, H., Zhang, X., Huang, T., He, C., et al. (2026). [Visual Generation Unlocks Human-Like Reasoning through Multimodal World Models](https://arxiv.org/abs/2601.19834). arXiv:2601.19834.
- Cui, Y., Chen, H., Deng, H., Huang, X., Li, X., Liu, J., et al. (2025). [Emu3.5: Native Multimodal Models are World Learners](https://arxiv.org/abs/2510.26583). arXiv:2510.26583.

### 39-05 将视觉隐状态融入CoT的推理（论文）

- Wang, Q., Shi, Y., Wang, Y., Zhang, Y., Wan, P., Gai, K., Ying, X., & Wang, Y. (2025). [Monet: Reasoning in Latent Visual Space Beyond Images and Language](https://arxiv.org/abs/2511.21395). arXiv:2511.21395.

### 39-06 UAE：训练中理解与生成合为自编码器（论文）

- Yan, Z., Lin, K., Li, Z., Ye, J., Han, H., Wang, Z., Liu, H., Lin, B., Li, H., Xu, X., Xiao, X., Wang, J., Wang, H., & Yuan, L. (2025). [Unified Multimodal Models as Auto-Encoders](https://arxiv.org/abs/2509.09666). arXiv:2509.09666.

### 39-07 VisionBanana：用视觉生成统一理解任务（论文）

- Gabeur, V., Long, S., Peng, S., Voigtlaender, P., Sun, S., Bao, Y., Truong, K., Wang, Z., Zhou, W., Barron, J. T., Genova, K., Kannen, N., Ben, S., Li, Y., Guo, M., Yogin, S., Gu, Y., Chen, H., Wang, O., Xie, S., Zhou, H., He, K., Funkhouser, T., Alayrac, J.-B., & Soricut, R. (2026). [Image Generators are Generalist Vision Learners](https://arxiv.org/abs/2604.20329). arXiv:2604.20329.

### 39-08 Uni-3DAR：跨尺度3D结构的统一理解与生成（论文）

- Lu, S., Lin, H., Yao, L., Gao, Z., Ji, X., E, W., Zhang, L., & Ke, G. (2025). [Uni-3DAR: Unified 3D Generation and Understanding via Autoregression on Compressed Spatial Tokens](https://arxiv.org/abs/2503.16278). arXiv:2503.16278.

## 未写入公开正文的短名说明

- `36-06` 的 DOCX 正文出现 `DiLA: Disentangled Latent Action World Models` 短名，但未能唯一核实该精确题名或短名对应的公开一手来源；公开 Markdown 保留已核实的 latent action / latent action world model 来源。
- `ABot-World` / `ABot-PhysWorld`、`Being-H0.7`、`SkiLa` / `HyLaR` 等短名出现在 DOCX 或 OCR 线索中，但公开来源、作者、题名和链接无法唯一确认，因此未写入公开正文参考文献。
- `A2A`、`AdaGen`、`π_RL` / `Flow-Noise`、`Self Forcing`、`Solaris`、`ELT`、`Dreamer 4`、`Muddit`、`Matrix-Game 3.0`、`RTFM`、`VLA-MBPO`、`VideoWorld 2`、`ADM` / `ADM-v2`、`Temporal Straightening`、`LeWorldModel`、`WorldLM`、`LingBot-VA`、`Fast-WAM`、`TMoW`、`AVIC`、`UAE`、`VisionBanana`、`Uni-3DAR` 等已按 DOCX/OCR 线索和公开来源补入对应 Markdown。
