...
Scores 操作描述： Q 的维度是 t × KT 的维度是 d × 这是一个标准的矩阵乘法。
...
Scores 操作描述： 的维度是 1 × do · (Kcache)T K 的维度是乯 × d （转置后为 d × t)o 这是一次 " 向量一矩阵 " 乘法。
...
丫 = Softmax(Scores) · V · 操作描述： 注意力矩阵维度乯 × 矩阵维度 × do
...
操作描述： 权重的维度是 1 × （0 的维度是 t × do 这是一次 " 向量一矩阵 “ 乘法。
