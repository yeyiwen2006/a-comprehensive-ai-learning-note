...
反思与自纠：以Self-RAG为代表的方法会训练LLM输出特殊的反思Token，判定是否需要检索、检索到的内容是否相关，以及生成的内容是否有用。
...
实践中常采取稠密和稀疏索引结合的方法。以claude-mem为例，其部分版本同时使用Chroma语义索引与SQLite持久化、过滤和结果补全。候选数量、时间窗口与排序逻辑会随版本变化，因此在没有固定release或commit时，不应把某一版本的默认常量当作稳定接口。
...
- 温睦宁、林江浩、张伟楠、俞勇. (2025). [《动手学大模型智能体》](https://haa.boyuai.com/). 人民邮电出版社. ISBN 978-7-115-68638-1.
...
- thedotmack. (n.d.). [claude-mem GitHub repository](https://github.com/thedotmack/claude-mem). 访问于2026-09-02.
