...
1. 用户说：“我叫小明。” $\to$ 存入上下文（剩 2 空位）。
2. 用户说：“今天天气不错。” $\to$ 存入上下文（剩 1 空位）。
3. 用户说：“吃个苹果。” $\to$ 存入上下文（满员）。
...
summary并非简单把长文本变成短文本，否则会丢失大量细节。它应尽量保留继续任务所需的目标、约束、已完成工作和未解决问题。OpenAI Codex在上下文Token超过阈值后，会调用Responses API的`/responses/compact`端点，用一组更小但能代表原对话的input items替换旧输入；其中包含带加密内容的`type=compaction`项，以保留模型对原对话的潜在理解。早期Codex曾使用摘要消息替换旧输入，但当前实现并不是固定的“小项目列表”。

## 参考文献

- 温睦宁、林江浩、张伟楠、俞勇. (2025). [《动手学大模型智能体》](https://haa.boyuai.com/). 人民邮电出版社. ISBN 978-7-115-68638-1.
- OpenClaw. (n.d.). [OpenClaw GitHub repository](https://github.com/openclaw/openclaw). 访问于2026-09-02.
- thedotmack. (n.d.). [claude-mem GitHub repository](https://github.com/thedotmack/claude-mem). 访问于2026-09-02.
- Hugging Face. (n.d.). [Cache strategies](https://huggingface.co/docs/transformers/main/kv_cache). Transformers Documentation. 访问于2026-09-02.
