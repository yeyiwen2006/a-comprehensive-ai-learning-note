...
计算梯度： gt = Vo 一 1） 更新动量 （一阶矩估计）： mt = · m 仁 1 + （1 一） · - 是的指数移动平均值。 01 决定了过去梯度的 " 遗忘 “ 速度。 01 = 0 · 9 意味着当前的 mt 大约 90 ％ 来自过去的动量， 10 ％ 来自当前梯度。 （： 在一些经典实现中丿也写作 mt = 01 · m 1 +gt' Adam 采用的是 EM 形丿我们在此统一值用 EMA 形更新参数： t—l 一 · 7 参数更新的方向不再是 gt, 而是平滑后的 mto
