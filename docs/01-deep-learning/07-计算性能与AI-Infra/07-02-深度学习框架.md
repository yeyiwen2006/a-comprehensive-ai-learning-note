...
int main() 阶段 1： 定义过程 (Symb01ic Definition) / / 此时根本没有发生任何数学运算， 只有 “ 字符串 ” 被记录下来 computation—graph · push computation—graph · push— / / 定义： g = mult(e， f) computation—graph · push— < < std：： endl; std: :vector<Node> computation—graph; / / 定义： e 一 add(a， b) / / 定义： f = add(c， d) back({ " ADD " " ADD " "MULT" std: :cout < < “ > > > 图纸绘制完毕， 此时没有进行任何计算。
...
2： 编译 / 优 / / 这是符号式编程的杀手锏。 (CompiIation/Optimization) / / 编译器看到完整图纸后， 发现了一个惊人的优化机会！ std: :cout < < “ > > > 正在编译和优化 · < < std: :endl; / / 假设输入是常数： a = 1， b = 2， c = 3， d=4 / / 编译器发现： 所有的输入都是固定的！ / / 它可以直接把 （1 + 2） * （3 + 4） 优化成 21， 完全删掉中间步骤。 / / 优化后的图纸可能只剩下一条指令： std: std: std:： string optimized—instruction "Return 21 耻阶段 3： •.COUt < < •.COUt < < 执行 (Execution) “ > > > 开始执行： " < < std：： endl; optimized—instruction < < std：： endl;
...
data[gpu0] ℃ opyfrom(data[0:50]) fc1[gpuO] = FullcForward(data[gpuO], fc1_weight[gpuO]) fc2[gpuO] = FullcForward(fc1 [gpuO], fc2_weight[gpuO]) fc2_ograd[gpuO] = LossGrad(fc2[gpuO], Iabel[0:50]) fcl ograd[gpuO], fc2 wgrad[gpuO] = FullcBackward(fc2_ograd[gpuO]， fc2_weight[gpuO])， fc1_wgrad[gpuO] 一 FullcBackward(fc1_ograd[gpuO]， fcl weight[gpuO]) data = next_batch() fc2_wgrad[cpu] fc2 wgrad[gpuO] + fc2 wgrad[gpul] fc2_weight[cpu] - lr*fc12_wgrad[gpuO] fc2_weig ht[cpu] ℃ opytO( fc2_weight[gpuO]， fc2 weight[gpul]) fcl wgrad[cpu] fc1_wgrad[gpuO] + fcl_wgrad[gpul] fcl_weight[cpu] - fc1_wgrad[gpuO] fcl _weig ht[cpu] .copyto( fc1_weight[gpuO] fcl_weight[gpul]) data[gpu0] ℃ opyfrom(data[51:100]) fcl[gpul] = FullcForward(data[gpul], fcl_weight[gpul]) fc2[gpu1] = FullcForward(fc1 [gpul], fc2_weight[gpu1]) fc2_ograd[gpul] = LossGrad(fc2[gpu1], 《 abe | [ 51： 100D fcl ograd[gpul], fc2 wgrad[gpul] = FullcBackward(fc2_ograd[gpul] fc2_weight[gpu1])， fcl_wgrad[gpul] = FullcBackward(fc1_ograd[gpul]， fcl weight[gpul])
