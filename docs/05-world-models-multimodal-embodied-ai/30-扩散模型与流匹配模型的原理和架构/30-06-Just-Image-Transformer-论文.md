...
**潜在空间（Latent Space）已经规避了“高维诅咒”**。图像理论上位于非常大的空间中，原始像素空间（Pixel Space）的维度很高，但当前工业界的世界模型几乎全部采用 Latent Diffusion 架构。它们通过 VAE（变分自编码器）将极其高维的视觉/图像帧压缩到一个非常低维的潜在空间（Latent Space）$z\in\mathbb{R}^k$ 中。
