...
最筒单的是分割任务。 假设模型生成的图像是： G@.y) e [ 0， 255 ] 3 其中 G@, 表示像素 （的 RG B 颜色。 prompt 里规定类别 c 对应颜色 ‰， 那么解码时可以用最近颜色匹配： = arg min 》 G@, 一 q。 目 2 意思是： 对每个像素， 找它生成颜色最接近哪个类别颜色， 就把它归到那个类别。 实例分割稍微不同， 因为实例数量事先不知道， 不能提前在 prompt 里指定每个实例的颜色。 论文采用 per-class inference: 每次只让模型分割某一类， 例如 " 每个 garlic instance 用不同颜色表示 "。 模型自己给不同实例分配颜色， 然后后处理时根据颜色聚类， 把不同颜色区域拆成不同实例。

> [图片 2：原 Word 此处有图片；为避免版权风险，开源版暂不上传图片。]

**图片文字 OCR（自动识别，待校对；数学公式必须人工核对）：**

深度估计更复杂， 因为深度是实数： d e [ 0， 到而 RG B 是有限范围： RGB e [ 0， 1 ] 3 所以作者设计了一个可逆映射， 把真实深度 d 编码成 RG B 颜色。 论文使用 Barron 的 P owe r transform 先把深度压缩到其中： d: 真实 metric depth, 单位是米。 A: 形状参数， 论文实验中取一 3。 c： 尺度参数， 论文实验中取 10 / 3。 歹@， c)： 归一化后的深度值， 用来映射到 RG B 色彩路径上。 这样做的原因是： 近处物体的深度误差通常更重要， 所以映射会给近距离分配更高分辨率， 远距离则被压缩。 随后， 论文把这个归一化深度沿 RG B 立方体边缘映射成颜色。 推理时， 模型生成彩色深度图， 再通过反函数把颜色解码回米制深度。
...
表面法线更直接。 法线是单位向量： RGB 通道是 [ 0， 1 ]， 所以可以用线性变换： 也就是： 2 生成图像后再反解： 这样 RG B 图就可以表示每个像素的表面朝向。 RGB = 佑 z [ n + 1 2 2 z + 1 2 n = 2RGB 一 1
