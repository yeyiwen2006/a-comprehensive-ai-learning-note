...
假设有 3 个视频块 $B_1,B_2,B_3$，去噪总步数 $S=4$，且噪声水平满足 $t_1>t_2>t_3>t_4>0$。推理序列如下：
...
论文在算法中指出，在第 $j+1$ 步时，第 $j$ 步的第 $n$ 个块 $B_n$ 只依赖于第 $j$ 步它前面的块 $B_{<n}$，以及它自己上一阶段（第 $j-1$ 步）的状态。因此，在 $N\times S$ 的去噪网格中，位于同一条反对角线上的任务是相互独立的，可以同时计算。
