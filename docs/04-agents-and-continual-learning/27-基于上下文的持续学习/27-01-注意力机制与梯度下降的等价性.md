...
假设我们正在进行 In-Context Learning（上下文学习），输入是一系列示例（演示数据）：$(x_1,y_1),(x_2,y_2),\ldots,(x_N,y_N)$，以及一个新的查询输入 $x_{test}$。
...
**输入 Token：**假设将输入特征 $x_i$ 和目标标签 $y_i$ 拼接成一个列向量作为 token，即：
...
$$
e_{test}=\begin{pmatrix}x_{test}\\y_{init}\end{pmatrix}
$$
...
![动态模型通过梯度步骤更新权重](../../../assets/images/04-agents-and-continual-learning/27-01/img-adf3e22406aa-0013.png)
![固定权重线性层通过数据流更新等价实现动态模型](../../../assets/images/04-agents-and-continual-learning/27-01/img-adf3e22406aa-0014.png)
- 穿过第 1 层注意力层（执行第 1 步梯度下降）：第 1 层根据当前的上下文计算出梯度增量 $\Delta W_1$。残差连接将其加上：

$$
y_{1,test}=y_{0,test}-\Delta W_1x_{test}=-(W_0+\Delta W_1)x_{test}
$$

- 穿过第 2 层注意力层（执行第 2 步梯度下降）：第 2 层读取到 $y_{1,test}$，继续计算出新的梯度增量 $\Delta W_2$。残差连接再次将其加上：

$$
y_{2,test}=y_{1,test}-\Delta W_2x_{test}=-(W_0+\Delta W_1+\Delta W_2)x_{test}
$$

- 穿过第 $K$ 层注意力层（执行第 $K$ 步梯度下降）：以此类推，经过 $K$ 层后，测试 Token 的 $y$ 值变成了：

$$
y_{K,test}=-W_0x_{test}-\sum_{k=1}^{K}\Delta W_k x_{test}
$$

$$
\hat{y}_{final}=-y_{K,test}=W_0x_{test}+\sum_{k=1}^{K}\Delta W_k x_{test}
$$
...
$$
Input=[(x_1,y_1),(x_2,y_2),\ldots,(x_N,y_N),(x_{test},y_{init})]
$$
...
1. 找一个替罪羊（Sink Token）：模型通常会选中序列的第一个 Token（比如 `<s>`）或者第一个标点符号。
2. 把它的 Value 变成 0：模型在训练参数时，偷偷把提取这个替罪羊 Value 的权重矩阵学成一种特殊状态，使得这个替罪羊的 $V_{sink} \approx 0$。
3. 倾泻无用的注意力：当模型在某一层“不想关注任何上下文”时，它就强行把高达 99% 的注意力分数（Attention Score）打给这个替罪羊。
...
**定义 1（联想记忆）**：给定一组键（Keys）$\mathcal{K}\subseteq\mathbb{R}^{d_k}$ 和值（Values）$\mathcal{V}\subset\mathbb{R}^{d_v}$，联想记忆是一个算子 $\mathcal{M}:\mathcal{K}\to\mathcal{V}$，用于映射这两组集合。为了从数据中学习这种映射，我们定义一个目标函数 $\tilde{\mathcal{L}}(\cdot;\cdot)$ 来衡量映射的质量，$\mathcal{M}$ 可以定义为：
...
* Nested Learning 的记忆过程：模型会把这次经历存进 MLP：“当你下次看到 'The sky is...'（$x$）时，记得要把那个叫 $u$ 的修正指令调出来。”
