...
ocr: "manual reconstruction completed; text screenshots transcribed as Markdown"
...
1. 用户说：“我叫小明。” -> 存入上下文（剩 2 空位）。
2. 用户说：“今天天气不错。” -> 存入上下文（剩 1 空位）。
3. 用户说：“吃个苹果。” -> 存入上下文（满员）。
...
前面提到过，当对话超出上下文限制时，让LLM为之前的对话生成一段摘要，注入之后的上下文窗口。写成程序语句如`summary=agent.ask(massage)`，`massage`里包含了要求生成摘要的prompt，这里的`summary`就暂时存在内存中，等待被注入新的上下文。
