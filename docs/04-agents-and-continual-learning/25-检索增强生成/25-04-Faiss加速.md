...
1. 聚类：在训练阶段，利用 K-means 算法将整个向量空间划分为 $k$ 个聚类（Voronoi Cells）。每个聚类中心被称为一个 Centroid。

2. 建立倒排表：每一个原始向量都会被归类到离它最近的一个或多个聚类中心下，形成一个“倒排列表”。

3. 查询加速：检索时，查询向量 $q$ 首先与 $k$ 个聚类中心对比，选出最近的 $nprobe$ 个聚类，只在这几个聚类下的候选向量中进行精细计算。
...
1. 在每一层执行贪婪搜索：检查当前节点的邻居，找到离 $\mathbf{v}_{new}$ 最近的一个邻居。

2. 将该邻居作为下一层的起始点，继续下沉。

3. 注意：在进入 $l$ 层之前，这个过程不建立连接，只为了快速定位。
...
1. 确定候选者：在当前层进行类似检索的搜索，维护一个大小为 $efConstruction$ 的候选节点集合。

2. 选择邻居（$M$ 个）：从候选中选出 $M$ 个点与 $\mathbf{v}_{new}$ 建立双向边。

3. 启发式连接策略（Heuristic）：这是 HNSW 的关键。系统不一定只选最近的 $M$ 个点，而是会考虑这些点是否“扎堆”。
