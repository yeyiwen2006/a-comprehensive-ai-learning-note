...
2. 采用了动量法，原来的 MLP 为 $\mathcal{M}_{t-1}$ 时，利用梯度的移动平均 $S_t$ 更新 MLP 的参数：
...
- **分形结构**：HOPE 模型不再区分“训练阶段”和“推理阶段”。它由多层 MLP 组成，每一层的更新速度不同：$f_1$ 高频，可能每读一个 token 就更新一次；$f_2$ 中频，可能每读 16 个 token 更新一次；$f_k$ 低频，可能每读 1M 个 token 更新一次，接近传统的预训练。
- **数学意义**：公式 (31) 定义了一个多时间尺度的在线梯度下降。每个 MLP 都在压缩属于它那个时间窗口内的上下文信息。这模仿了人类大脑：只有极其重要或重复的信息（通过慢速层）才会变成长期记忆，而原本的信息流（快速层）会被快速处理并遗忘。
