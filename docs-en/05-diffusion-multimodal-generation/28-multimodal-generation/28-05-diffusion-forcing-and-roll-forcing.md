---
title: "28.5 Diffusion Forcing and Roll Forcing"
chapter_title: "Multimodal Generation"
section_id: "28-05"
language: en
source_language: zh
source_docx: "第5部分 扩散模型与多模态生成/28.多模态生成/28.5 Diffusion Forcing与Roll Forcing.docx"
...
# 28.5 Diffusion Forcing and Roll Forcing
...
1. Sample a real observation sequence $(x_1,\ldots,x_T)$ from the dataset.
2. Assign independent noise: for $t=1,\ldots,T$, independently and uniformly sample a noise level $k_t\in\{0,1,\ldots,K\}$ for each $x_t$.
3. Forward noise addition: diffuse each token to its assigned noise level, obtaining observation $x_t^{k_t}$ and computing target noise $\epsilon_t$.
4. Hidden-state update and noise prediction: predict noise from the previous hidden state $z_{t-1}$, current noisy observation $x_t^{k_t}$, and noise level $k_t$:
...
5. Backpropagation: compute MSE between predicted and actual noise at all time steps and update network weights $\theta$.

### (3) Inference: control through a scheduling matrix
...
1. Global initialization: initialize the entire target sequence $x_{1:T}$ as independent Gaussian white noise, with every token at noise level $K$.
2. Outer loop (row-wise denoising): traverse the matrix rows, for example from $M$ down to $0$. Each row is one global denoising update.
3. Inner loop (causal progression): within row $m$, traverse time $t=1,\ldots,T$ from left to right.
4. Read the target and denoise one step: for $(m,t)$, read $k=K_{m,t}$. Using current hidden state $z_{t-1}$ and $x_t$ with its actual current noise, predict $\epsilon_\theta(z_{t-1},x_t,k)$, then use a Langevin dynamics formula or DDIM to reduce $x_t$ precisely to target noise level $k$.
5. Causal state update: update hidden state $z_t$ from the newly denoised $x_t$. Under the causal architecture, updated $z_t$ contains the latest information about $x_{1:t}$ and passes to time step $t+1$.

### (4) Generation paradigms based on the scheduling matrix

Scheduling matrix $K$ supports three typical paradigms.

#### 1. Full-sequence diffusion mode
...
#### 3. Causal uncertainty or pyramid scheduling
...
**Correspondence with Transformer architectures:**
...
### (5) Planning with Monte Carlo guidance
...
1. Current observation: at the environment's current real time step, the agent has the historically accumulated hidden state $z_{t-1}$.
2. Generate a look-ahead window: instead of planning indefinitely, set a future horizon $H$. The model begins generating a local future sequence $x_{t:t+H}$ in latent space.
3. MCG sampling at each denoising step: during every denoising iteration of local sequence $x_{t:t+H}$, to decide how to denoise the current $x_t^k$, branch into multiple uncertain future trajectories in latent space from $t+1$ to $t+H$.
...
### (6) Why can this architecture send future gradients into the past when conventional autoregression and diffusion cannot?
...
1. Future noise is much higher than current noise. The future is therefore not locked in: we can freeze the current $x_t^k$ and inject hundreds or thousands of different white-noise samples into the future to produce enough branches.
2. The current token has not been hard-committed as in autoregression and remains slightly noisy, for example at $k=2$. As a continuous, uncertain variable, it can receive the expected-reward gradient directly, smoothly changing its denoising direction in subsequent Langevin dynamics.

### (7) Slightly noising historical information
...
### (8) Proof of theoretical feasibility
...
## II. Rolling Forcing: Rolling Denoising Generation
...
1. **Joint denoising in a sliding window**: instead of denoising frame by frame, denoise jointly within a sliding window. The $T$ frames in the window receive increasing noise levels, from almost clear to pure noise. Bidirectional attention lets relatively clear frames guide highly noisy ones, suppressing errors locally before they propagate to the next frame. Each forward pass outputs only the clearest frame, then shifts the whole window forward.

![Comparison of sequence generation in Diffusion Forcing and Rolling Forcing](../../../assets/images/05-diffusion-multimodal-generation/28-05/diffusion-forcing-rolling-forcing-comparison.png)

![Meaning of the colors and attention connections in the diagram](../../../assets/images/05-diffusion-multimodal-generation/28-05/diffusion-forcing-rolling-forcing-legend.png)

2. **Attention sink**: cache the key-value states of the first few video frames as “global context anchors,” ensuring that, however far generation proceeds, the model can look back at the initial state. This strongly preserves global style, color tone and related information.

3. **Hybrid training**: combine RF's rolling-window objective with SF's frame-by-frame objective. SF acts as a regularizer here, preventing unnatural camera motion caused by the rolling-window mechanism.
...
- Liu, K., Hu, W., Xu, J., Shan, Y., & Lu, S. (2025). [Rolling Forcing: Autoregressive Long Video Diffusion in Real Time](https://arxiv.org/abs/2509.25161). ICLR.
