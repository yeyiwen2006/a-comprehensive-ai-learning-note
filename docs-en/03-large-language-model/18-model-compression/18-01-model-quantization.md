...
# 18.1 Model Quantization

## I. The Core Idea of Model Quantization
...
2. Normalize the weights by dividing each original weight by the scale factor $S$, obtaining the normalized weights $R_{norm} = \frac{R}{S}$.

   - $0.45 / 2.10 = 0.214$
   - $-0.88 / 2.10 = -0.419$
   - ...
   - This gives the normalized tensor $R_{norm}$: $[0.214, -0.419, 0.733, -0.057, 0.024, 0.471, -0.157, -1.0]$

3. Map each value to the nearest NF4 quantization point and convert it to the corresponding 4-bit NF4 index.

4. Store the results. After quantization, two items ultimately need to be stored:
...
1. Read the indices and scale factor.

   - Read the previously stored index array and scale factor $S = 2.10$ from memory.

2. Recover the normalized values from the quantization table: use the indices to look up the corresponding floating-point values in the NF4 quantization table.

   - $10 \to 0.1906$
   - $3 \to -0.3949$
   - ...
   - This gives the recovered normalized tensor $R'_{norm}$: $[0.1906, -0.3949, 0.6717, -0.0911, 0.0062, 0.4045, -0.1848, -1.0]$

3. Multiply by the scale factor to recover the weights: multiply the recovered normalized values by the scale factor $S$ to obtain the final dequantized weights $R'$.
...
## IV. Static and Dynamic Quantization

### 1. Static Quantization

(1) Core idea and procedure
...
(2) Advantages, disadvantages, and uses
...
(2) Advantages, disadvantages, and applications
...
- Jacob, B., Kligys, S., Chen, B., et al. (2018). [Quantization and Training of Neural Networks for Efficient Integer-Arithmetic-Only Inference](https://arxiv.org/abs/1712.05877). CVPR 2018.
...
- Dettmers, T., Pagnoni, A., Holtzman, A., & Zettlemoyer, L. (2023). [QLoRA: Efficient Finetuning of Quantized LLMs](https://arxiv.org/abs/2305.14314). NeurIPS 2023.
