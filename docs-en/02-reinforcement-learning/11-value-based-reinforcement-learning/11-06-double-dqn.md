...
- **Equal true values**: at state $s$, all actions $a$ have the same true expected return $Q^{*}(s,a)$, denoted $V^{*}(s)$.
- **Reason for this assumption**: if true values differ, selecting the largest by $\max$ is natural. The aim is to show that even with equal true values, noise alone makes $\max$ overestimate.
- **Noise distribution**: estimation errors $\epsilon_a=Q_{w^{-}}(s,a)-V^{*}(s)$ are uniform on $[-1,1]$, with $\mathbb{E}[\epsilon_a]=0$.
- **Independence**: errors for different actions are independent.
- **Number of actions**: the action space has size $m$.
...
- **Truth**: its value is $0$.
- **Noise**: uniform on $[-1,1]$. It may be estimated as $0.8$ (overestimated) or $-0.9$ (underestimated).
