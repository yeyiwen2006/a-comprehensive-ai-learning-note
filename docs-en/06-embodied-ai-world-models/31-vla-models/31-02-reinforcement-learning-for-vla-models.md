---
title: "31.2 Reinforcement Learning for VLA Models"
...
# 31.2 Reinforcement Learning for VLA Models

## I. Challenges of Reinforcement Learning for VLA Models
...
## II. VLA-RL: Directly Optimizing the Entire VLA with PPO
...
### (2) Sparse Feedback and Its Solution
...
## III. πRL: Enabling Whole-Model RL Training for Flow-Matching VLAs
...
**Step 1: Environmental interaction and initialization (start of the outer loop)**

1. The agent obtains the observation state $o_t$ at the current time step $t$ from a simulated environment or the real world, such as camera images and proprioceptive data.
2. Inside the model, initialize the flow-matching starting point by sampling $A_t^0\sim\mathcal{N}(0,I)$ from standard Gaussian noise.

**Step 2: Discretized denoising (the internal MDP loop)**

Start a discrete denoising loop from $\tau=0$ to $\tau=1$ with step size $\delta$. At each step:

1. Construct the internal state by combining the environmental observation and current variable as $\bar{s}_t^\tau=(o_t,A_t^\tau)$.
2. Network inference: Feed internal state $`\bar{s}_t^\tau`$ into the VLA backbone (such as a Transformer), predict the current velocity field $`v_\theta`$, and compute the expected denoising direction $\mu_\tau$.
3. Noise evaluation: The auxiliary learnable noise network outputs standard deviation $\sigma_\tau$ based on the current state.
4. Sample and record: Sample random noise $\varepsilon$ and compute the next latent $A_t^{\tau+\delta}=\mu_\tau+\sigma_\tau\delta\cdot\varepsilon$. At the same time, record the step's log-likelihood $\log p_\theta(A_t^{\tau+\delta}\mid A_t^\tau,o_t)$ in the buffer.

**Step 3: Action execution and reward collection (end of the outer loop)**

1. When the internal denoising loop reaches $\tau=1$, obtain the noiseless deterministic physical action $a_t=A_t^1$.
2. Send action $a_t$ to the environment for execution.
3. Update the observation state and return a single-step reward $r_t$, such as a sparse $+1$ reward for a successful grasp.

**Step 4: Reinforcement-learning policy update (RL backpropagation)**

1. After collecting complete trajectories for one or more episodes, compute the cumulative advantage function.
2. Extract the exact sum of joint log-likelihoods $\sum\log p_\theta$ computed in Step 2.
3. Use PPO's clipped surrogate objective to compute policy gradients.
4. Update the VLA and noise-network parameters to maximize the probability of generating denoising trajectories that achieve high environmental rewards under similar observations.

### (4) How This Method Addresses Flow Matching's “Exploding Step Count after Departing from Optimal Transport”
...
## IV. Policy Agnostic RL: Letting RL Gradients Optimize Output Actions Instead of the VLA Model
...
## V. DSRL: Training Only “Which Noise to Choose,” Not the VLA
...
### (2) Noise Aliasing in Critic Training
...
## VI. RLDG: Using RL Only to Produce Supervised-Learning Samples
